...
<c:set var="taskLevelPriceInd" value="${taskLevelPriceInd}" scope="session" />
...
<head> 
    ...
    <body>
                        	...
                        	<div id="checkspend" class="divContainerUp"
                                ...
                                <div align="center">
                                        <div class="modalHomepage">
                                                <div style="float:left;">Increase Price</div>
                                                <a href="javascript: void(0)" class="btnBevel simplemodal-close" style="color: white;" float:right>Close</a>
                                        </div>
                                        <div class="modalheaderoutline">
                                                <div class="rejectServiceOrderFrame" style="width: 100%; border: 0px">
                                                        ...
                                                        <div class="rejectServiceOrderFrameBody" style="width: 300px;">
                                                                ...
                                                                then click "submit". You may provide a
                                                                reason for the increase.
                                                                <br />
                                                                <input type="hidden" name="buyerId" id="buyerId"
																		value="${currentSO.buyerId}" />
																...
																<input type="hidden" name="currentTab" id="currentTab"
																		value="${currentTab}" />
																		<input type="hidden" name="increasedSpendLimitReason" id="increasedSpendLimitReason"
																		value="" />	
																<input type="hidden" name="increasedSpendLimitReasonId" id="increasedSpendLimitReasonId"
																		value="" />	
        ...
        <table>
          ...
          <div style="overflow-y: auto; overflow-x: hidden; height: 150px"> 
                ...
                <c:forEach var="task" items="${data.tasks}" varStatus="rowCounter">
                <c:if test="${!( task.taskStatus!=null && (task.taskStatus=='CANCELED' ||task.taskStatus=='DELETED'))}">
                <c:set var="count" value="${count+1}"/>
                <input type="hidden" value="${task.sku}" id="taskList[${index}].sku" name="taskList[${index}].sku"/>
                <input type="hidden" value="${task.taskId}" id="taskList[${index}].taskId" name="taskList[${index}].taskId"/>
               	<input type="hidden" value="${task.taskType}" id="taskList[${index}].taskType" name="taskList[${index}].taskType"/>
                <input type="hidden" value="${task.sequenceNumber}" id="taskList[${index}].sequenceNumber" name="taskList[${index}].sequenceNumber"/>
                <c:if test="${task.sequenceNumber == 0 || task.sequenceNumber == seqNum}">
                      <input type="hidden" id="taskList[${index}].finalPrice" name="taskList[${index}].finalPrice" value="0.00"/>
                </c:if>
               	<c:if test="${task.sequenceNumber != 0 && task.sequenceNumber != seqNum}">	                                                        
                <tr style="height: 20px" >
                            <c:set var="taskNumber" value="${taskNumber+1}"/> 
                            	<td style="align: left; width:190px; padding-bottom: 15px;padding-left: 8px">	
                                ...
                                <td style="padding-bottom: 15px;">
                                        ...
                                        <td>
                                                ...
                                                <input type="hidden" id="taskList[${index}].finalPrice" name="taskList[${index}].finalPrice" value="${task.sellingPrice}"/>
                                        </td>
                                        </c:if>
                                        <c:if test="${task.taskType == 0}">
                                        <td>
                                                <input type="text" onFocus="findPos(this,'checkspend')"
                                                        onblur="findMaxLabor(this);" id="taskList[${index}].finalPrice" size="9"        
                                                        value="<fmt:formatNumber value="${task.finalPrice}" type="NUMBER" minFractionDigits="2" maxFractionDigits="2" />"  name="taskList[${index}].finalPrice"
                                                        class="textbox-60 alignRight shadowBox" />
                                        </td>
                                        <c:set var = "totalTaskPrice" value = "${totalTaskPrice + task.finalPrice}"/>
                                        </c:if>
                                </td>
                                
                                <td>
                                <c:if test="${fn:length(task.priceHistoryList) > 1}">
									<a id = "${count}" class="priceHistoryIcon" href="javascript:void(0);"><img src="${staticContextPath}/images/widgets/dollaricon.gif"/></a>
                                </c:if>
                                </td>
 ...
 <c:set var = "seqNum" value  = "-1"/>           
   ...
   <c:if test="${!( task1.taskStatus!=null && (task1.taskStatus=='CANCELED' ||task1.taskStatus=='DELETED'))}">
   <c:set var="countHist" value="${countHist+1}"/>
   <c:if test="${task1.sequenceNumber != 0 && task1.sequenceNumber != seqNum}">
   <c:set var="taskNumberHis" value="${taskNumberHis+1}"/>
		<div id ="${countHist}priceHistory" class="priceHistory" style="position:absolute;
			left:400px;
			top:800px;
			width:215px;
...
<input type="hidden" value="" id="taskErrorCount" name="taskErrorCount"/>
 <input type="hidden" id="currentLimitLaborAmt" name="currentLimitLaborAmt" value="${currentSO.spendLimitLabor}"/>   

<table style="border-bottom: #ccc 1px solid;;" width="300">
                                                                        <tr >
                                                                        		<td width="190" align="right" style="padding-bottom: 15px;padding-left: 8px;">
                                                                                ...
                                                                                <td align="left" style="padding-bottom: 15px;">
                                                                                        <div id="maxLabor"><fmt:formatNumber value="${totalTaskPrice}" type="NUMBER" minFractionDigits="2" maxFractionDigits="2" /></div>
                                                                                </td>
                                                                                <input type="hidden" name="totalSpendLimitAmt"
                                                                                        id="totalSpendLimitAmt" />
                                                                        ...
                                                                        <tr>	
                                                                                ...
                                                                                <td>
                                                                                        <input type="hidden" id="currentLimitPartsAmt"
                                                                                                name="currentLimitPartsAmt" value="${currentSO.spendLimitParts}"/>
                                                                                </td>
                                                                        </tr>
                                                                        </table>
                                                         <table style="border-bottom: 1px solid #ccc; width: 300px;">
                                                                        <tr>
                                                                                ...
                                                                                <td align="center" style="padding-bottom: 15px;">                                                                                
                                                                                        <div id="totalAmt"><fmt:formatNumber value="${currentSO.spendLimitLabor+currentSO.spendLimitParts}" type="NUMBER" minFractionDigits="2" maxFractionDigits="2" /></div>
                                                                                </td>
                                                                                 <td width="10"></td>
                                                                        </tr>
                                                        </table>
                                                        <br/>
                                                        <br /> <b>Reason <font color="red">*</font> : </b> <br/>
														<c:choose><c:when test="${fn:length(spendLimitReasonCodes) > 0}">
                                                        <select name="reason_wfm" id="reason_wfm" width="300px"
																style="width: 300px;border:2px solid #CCCCCC"  onchange="fnEnable();">
														...
														<br/>
														<br/>
                                                                <b>Notes </b><i>(Required if 'Other' is selected above)</i> :
                                                                <br/>
                                                                <textarea id="comment_wfm"
                                                                        name="comment_wfm" class="shadowBox"
                                                                        style="width: 300px; background: #E3E3E3;" onkeyup="limitText(this,255);" onkeydown="limitText(this,255);" disabled="disabled" ></textarea>
                                                                <br />
														</c:when>
                                                        <c:otherwise>
                                                        <br/>
                                                        <br/>
                                                                <textarea id="comment_wfm"
                                                                        name="comment_wfm" class="shadowBox"
                                                                        style="width: 300px;" onkeyup="limitText(this,255);" onkeydown="limitText(this,255);" ></textarea>
                                                        </c:otherwise></c:choose>
                                                                <br />
                                                                <a id="submitIncSpendLimitBtn"><img
                                                                                ...
                                                                                class="btnBevel" /> </a>
                                                                <a href="#" style="float: left; padding-right: 18px;color: red;" class="btnBevel simplemodal-close" > Cancel </a>
    ...
    jQuery(document).ready(function ($) {
			...
			$('#submitIncSpendLimitBtn').click(function(){
                ...
                var maxParts = parseFloat($('#increaseLimitParts').val());
                var reasonCode = "";
                var reasonCodeId = "";
  			  	if($('#reason_wfm').val()!= null){
  			  	 reasonCodeId = document.getElementById('reason_wfm').value;
  			  	 var selected_index = document.getElementById('reason_wfm').selectedIndex;
  			 	 reasonCode = document.getElementById('reason_wfm').options[selected_index].text;
  			  	}                
  			  	var reasonComment = document.getElementById('comment_wfm').value;
            ...
            if(success){
               ...
               //newco.jsutils.doAjaxSubmit('incSpendLimitAction.action', updateOrderExpressMenuWithPrice,null,'frmIncreaseSpendLimit');
               }
				}
             });
    ...
    function fnEnable(){
    	var reasonCode = document.getElementById('reason_wfm').value;
    	if(reasonCode == "-2"){
    		document.getElementById('comment_wfm').disabled = false;
    		document.getElementById('comment_wfm').value = "";
    		document.getElementById('comment_wfm').style.background = '#FFFFFF';

    	}else{
    		document.getElementById('comment_wfm').disabled = true;
    		document.getElementById('comment_wfm').value = "";
    		document.getElementById('comment_wfm').style.background = '#E3E3E3';
...
function findMaxLabor(obj)
  ...
  function fnCalcMaxLabor(){
        ...
        function fnCalcTotalAmt(){
                ...
                var maxLabor = parseFloat(document.getElementById('totalSpendLimitAmt').value);
                var $resolveParts = jQuery.noConflict();
                var maxParts = parseFloat($resolveParts('#increaseLimitParts').val());
                var valid = true;
                var partsPrice=$resolveParts('#increaseLimitParts').val();
                ...
                document.getElementById('totalSpendLimitParts').innerHTML = parseFloat(maxParts);
                return "true";
                }
        }
        </script>
