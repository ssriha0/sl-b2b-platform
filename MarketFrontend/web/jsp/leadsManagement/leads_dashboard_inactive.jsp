...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
	<header>
             <div class="col-sm-8 col-md-3">
                 <h1>Inactive Leads</h1> <span class="badge tooltip-target" data-toggle="tooltip" data-placement="bottom" data-original-title="Total Inactive leads">${totalInActiveLeadCount}</span>
            ...
            <c:if test="${fn:length(inActiveLeadDetails)>0}">
			<c:forEach var="leadDetail" items="${inActiveLeadDetails}" varStatus="status">
		  	<c:choose>
		  	 <c:when test="${leadDetail.leadStatus== 'completed'}">
		  	<c:set var="className" value="list-group-item"/>
		  	<c:set var="iconName" value="icon-leadStatus-4"/>
		  </c:when>
		  <c:when test="${leadDetail.leadStatus== 'cancelled'}">
		  	<c:set var="className" value="list-group-item"/>
		  	<c:set var="iconName" value="icon-leadStatus-5"/>
		  </c:when>
		  <c:when test="${leadDetail.leadStatus== 'stale'}">
		  	<c:set var="className" value="list-group-item"/>
		  	<c:set var="iconName" value="icon-leadStatus-6"/>
		  </c:when>
		  	</c:choose>
...
<!-- View more -->
             <c:choose>
             <c:when test="${totalInActiveLeadCount>inactiveLeadDetailsCount}">
             		<div id ="viewStyle" style="margin-left:1.3%">
             		<p>Showing ${inactiveLeadDetailsCount} of ${totalInActiveLeadCount}</p>
             		</div>
            		<a href="#" id="viewMoreInActiveOrders" class="btn" role="button" onclick="viewMoreInActiveLeads();"><u>View More Leads</u></a>
           	</c:when>
           	</c:choose>
