...
jQuery(document).ready(function () {
	...
	$('#listHead').html('Provider Firm Exclusion List');
	$('#addnMsg').html('');
});
</script>
					<c:if test="${fn:length(firmExclusionDTOList) > 0 }">
					  ...
					  <table style="margin-top: 1%;" cellpadding="0" cellspacing="0" >
					    ...
					    <c:if test="${fn:length(firmExclusionDTOList) == 0 }">
					    	<tr><td colspan="5">No Firms Added to Exclusion List</td></tr>
					    </c:if>
				...
				<tr class="scrollerTableRow">
					<td style="padding: 15px;">
						<input class="autoCloseCheck" type="checkbox"
							value="${firm.autoCloseRuleAssocId}" onclick="autoCloseCheck(this);" />
					</td>
					
					<td width="14%">
						<a href="javascript:openFirm(${firm.providerFirm.id});" id="firmID${firm.providerFirm.id}">
							...
							style="margin-right: 10px;" />
								...
								<div class="comHistLayer scroll" id="comHistLayer${count}" style="display: none;">
												<c:forEach var="firmHistory" items="${firm.firmAssocHistory}">
													...
													Comments: ${firmHistory.autoCloseRuleFirmExcludeReason}<br>
													-----------------											
												</c:forEach>
								</div>
							</c:if>
								...
								Action: Added
								<br>
								Date: ${firm.excludedDateFormatted}
								<br>
						...
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ${firm.excludedDateFormatted}
							...
							<tr class="reasonText" id="txt${firm.autoCloseRuleAssocId}" style="display: none;" >
								<td colspan="4"><textarea id="comment${firm.autoCloseRuleAssocId}" class="shadowBox grayText" style="width: 600px;" maxlength="750" onkeydown="CheckMaxLength(this, 749);">Enter reason for removing from the exclusion list</textarea></td>
  						        <td style="float: left;"> 
  						        	<!-- <input type="image" id="removeFirm${firm.autoCloseRuleAssocId}" src="${staticContextPath}/images/common/spacer.gif" width="72" height="20" style="background-image:url(${staticContextPath}/images/btn/remove.gif);" class="btn20Bevel" />  -->
  						        	<input type="button" value="REMOVE FROM EXCLUSION LIST" class="button action btn20Bevel" id="removeFirm${firm.autoCloseRuleAssocId}" 
