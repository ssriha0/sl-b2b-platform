...
jQuery(document).ready(function () {
	...
	$('#listHead').html('Providers Removed from Exclusion List');
					  ...
					  <table style="margin-top: 1%;" cellpadding="0" cellspacing="0" >
					    ...
					    <c:forEach var="firm" items="${firmExclusionDTOList}">
							...
							<tr class="scrollerTableRow">
								...
								<input class="autoCloseCheck" type="checkbox" value="${firm.autoCloseRuleAssocId}" onclick="autoCloseCheck(this);"/>
								</td>
								
								<td width="14%"><a href="javascript:openProvider(${firm.serviceProvider.id},${firm.serviceProvider.providerFirm.id});" id="provID${firm.serviceProvider.id}-${firm.serviceProvider.providerFirm.id}"><u>${firm.serviceProvider.contact.firstName} ${firm.serviceProvider.contact.lastName}</u></a></td>
							...
							<tr class="reasonText" id="txt${firm.autoCloseRuleAssocId}" style="display: none;">
								<td colspan="4"><textarea id="comment${firm.autoCloseRuleAssocId}" class="shadowBox grayText" style="width: 600px;" maxlength="750" onkeydown="CheckMaxLength(this, 749);" >Enter reason for adding to the exclusion list</textarea></td>
  						        <td style="float: left;"> 
  						        	<!-- <input type="image" id="addProv${firm.autoCloseRuleAssocId}" src="${staticContextPath}/images/common/spacer.gif" width="35" height="20" style="background-image:url(${staticContextPath}/images/btn/add.gif);"class="btn20Bevel" />  -->
  						        	<input type="button" value="ADD TO EXCLUSION LIST" class="button action btn20Bevel" id="addProv${firm.autoCloseRuleAssocId}" 
