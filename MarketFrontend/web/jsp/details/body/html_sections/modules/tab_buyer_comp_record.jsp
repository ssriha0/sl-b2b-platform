...
<c:set var="staticContextPath" scope="request" value="<%=request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+"/ServiceLiveWebUtil"%>" />
            <jsp:include page="/jsp/public/common/omitInclude.jsp" flush="true">                  <jsp:param name="PageName" value="ServiceOrderDetails.buyerCompletionRecord"/>            </jsp:include><html>
<head>
...
<script type="text/javascript">
      jQuery("#buyerCompRecordlink").load("soDetailsQuickLinks.action?soId=${SERVICE_ORDER_ID}");  
      function clickCompInfo(path)
{
    jQuery("#buyercomprec p.menugroup_head").css({backgroundImage:"url("+path+"/images/widgets/titleBarBg.gif)"}).next("div.menugroup_body").slideToggle(300);
...
<c:if test="${Message == 'This order was successfully closed and payment has been made'}">
...
<div id="autocloseinfo" class="menugroup_list">
  <p class="menugroup_head" onClick="autocloseclickCompInfo('${staticContextPath}');">&nbsp;<img id="compImg" src="${staticContextPath}/images/widgets/arrowDown.gif"/>&nbsp;Auto Close Information</p>
   ...
   <c:if test="${SecurityContext.roleId == 3 || (SecurityContext.roleId == 1 && viewOrderPricing=='true')}">
<div id="buyercomprec" class="menugroup_list">
  <p class="menugroup_head" onClick="clickCompInfo('${staticContextPath}');">&nbsp;<img id="compImg" src="${staticContextPath}/images/widgets/arrowDown.gif"/>&nbsp;General Closing Information</p>
    <div class="menugroup_body">
		<p>
...
<th colspan="5" style="text-align: left;"><fmt:message bundle="${serviceliveCopyBundle}" key="details.completepayment.addon.msg2" />(customer paid on-site)**</th>
					...
					<td colspan="3"></td>
							...
							<c:when test="${addonServiceRowDTO.sku=='99888'}">
								...
								<c:if test="${ displayTax }">
									<td style="width:12%;text-align:right"><fmt:formatNumber value="${addonServiceRowDTO.endCustomerSubtotal * addonServiceRowDTO.taxPercentage  / (1 + addonServiceRowDTO.taxPercentage)}" type="currency" minFractionDigits="2" maxFractionDigits="2" currencySymbol="$"/></td>
								</c:if>
								<td style="width:15%;text-align:right" align="right"><label id="custChargeWithoutAddons"><fmt:formatNumber value="${addonServiceRowDTO.endCustomerSubtotal}" type="currency" minFractionDigits="2" maxFractionDigits="2" currencySymbol="$"/></label></td>
								</tr>
								</c:if>
								</c:forEach>
								<tr style="background: #CCCCCC; color: #FFF; border-bottom: 1px solid #CCCCCC;">
								...
								<td style="width:18%;color: #666;text-align:right" align="right"><fmt:formatNumber value="${soCloseDto.addonServicesDTO.paymentAmount-soCloseDto.permitTaskAddonPrice}" type="currency" minFractionDigits="2" maxFractionDigits="2" currencySymbol="$"/></label></td>
								<c:if test="${ displayTax }">
			...
			<table  class="globalTableLook"  style="margin-bottom: 0px;width:90%;" width="85%" cellspacing="0" cellpadding="0">
				<tr style="border-bottom: 1px solid #CCCCCC;">
								...
								<c:choose>
								<c:when test="${empty soCloseDto.permitTaskList && (soCloseDto.addonServicesDTO==null||soCloseDto.addonServicesDTO.endCustomerSubtotalTotal<=0)}">
								<td style="width:11%;background: #BDEDFF; color: #666;text-align:right" align="right"><label id="custChargeFinal" name="custChargeFinal"></label></td>
								</c:when>
								<c:otherwise>
								<td style="width:11%;background: #BDEDFF; color: #666;text-align:right" align="right"><label id="custChargeFinal" name="custChargeFinal"><fmt:formatNumber value="${soCloseDto.addonServicesDTO.endCustomerSubtotalTotal}" type="currency" minFractionDigits="2" maxFractionDigits="2" currencySymbol="$"/></label></td>
								</c:otherwise>
								</c:choose>
...
<!-- NEW MODULE/ WIDGET for Document Management-->
 <c:set var="docSource" scope="request" value="<%=(OrderConstants.CLOSE_AND_PAY) %>" />
