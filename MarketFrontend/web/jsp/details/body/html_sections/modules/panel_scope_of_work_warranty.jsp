		...
		<tr>
			<td class="column1">
				<p class="text11px">
					...
					<br />		
					<c:choose>
						<c:when test="${roleType == BUYER_ROLEID || roleType == SIMPLE_BUYER_ROLEID || (roleType == PROVIDER_ROLEID)}">
			...
			<p>
				...
				<c:choose>
					<c:when test="${fn:length(task.comments) > MAX_CHARACTERS_WITHOUT_SCROLLBAR}">
						<div class="inputArea" style="height: 200px;fixed-width: 600px;background:#EDEDED;word-wrap:break-word;" >
							${task.comments}
						</div>
					</c:when>
					<c:otherwise>
					<div style="fixed-width: 600px;word-wrap:break-word;">
						${task.comments}
					</div>
					</c:otherwise>
				</c:choose>
