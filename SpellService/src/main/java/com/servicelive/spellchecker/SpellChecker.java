...
public class SpellChecker {
	 ...
	 * 
	 * @param dictionary
	 *            The dictionary used for looking up words.
	 */
	 ...
	 */
	public SpellCheckResultDto checkString(String text, int count) {
		if (count == 0) count++;
		text = text.trim();
		text = text.replaceAll("\\+", " ");
		StringTokenizer tokenizer = new StringTokenizer(text);
		//Map<String, List<String>> suggestion = new LinkedHashMap<String, List<String>>();
		StringBuilder bldArr[] = new StringBuilder[count];
		SpellCheckResultDto result = new SpellCheckResultDto(false);
		
		while (tokenizer.hasMoreElements()) {
			String word = tokenizer.nextToken();
			StringWordTokenizer t = new StringWordTokenizer(word);
			Map<String, List<String>> suggestion = new LinkedHashMap<String, List<String>>();
			checkSpelling(t, suggestion, count);
			
			
			for (int i = 0; i < count ; i++) {
				StringBuilder bld = bldArr[i];
				if (bld == null) {
					bld = new StringBuilder();
					bldArr[i] = bld;
				}				
				List<String> list = suggestion.get(word);
				if (list == null) {
					 bld.append(word + " ");
				} else if (list.size() > i)
				  bld.append(list.get(i) + " ");
				else 
				  bld.append(list.get(0) + " ");
			}
		}	
		
		
		for (StringBuilder bb:bldArr) {
			if (bb != null)
				result.getSuggestions().add(bb.toString().trim());
		}
		
		String val = "";
		Iterator<String> r = result.getSuggestions().iterator();
		if (r.hasNext())
		  val = r.next();
		
		if (val.equals(text)) {
			result = new SpellCheckResultDto(true);
		}
		return result;
	}
	...
	public List<Word> getSuggestions(String word, int threshold) {
		...
		if (cache != null)
			suggestions = (ArrayList<Word>) cache.get(word);
		...
		if (suggestions == null) {
			...
			for (Enumeration<?> e = dictionaries.elements(); e.hasMoreElements();) {
				...
				if (dictionary != userdictionary)
					VectorUtility.addAll(suggestions, dictionary
							.getSuggestions(word, threshold), false);
			}
		...
		VectorUtility.addAll(suggestions, userdictionary.getSuggestions(word,
				threshold), false);
	 ...
	 */
	private final int checkSpelling(WordTokenizer tokenizer, Map<String, List<String>> map, int count) {
		int errors = 0;
				...
				if ((config.getBoolean(Configuration.SPELL_IGNOREMIXEDCASE) && isMixedCaseWord(
					...
					if (!isIgnored(word)) {
						...
						if (autoReplaceWords.containsKey(word)) {
							...
							// Fire the event.
							List<Word> suggestions = getSuggestions(word, config
									.getInteger(Configuration.SPELL_THRESHOLD));
							
							sugList = new ArrayList<String>();
							...
							if (suggestions.size() > 0)	 {
								if (count < 2) {
									sugList.add(suggestions.get(0).getWord());									
								} else {
									int end = count;
									if (suggestions.size() < count)
										end = suggestions.size();
									for (int i= 0; i < end; i++) {
										sugList.add(suggestions.get(i).getWord());
									}
								}
								map.put(word, sugList);
							}							
	...
	private List<Word> makeSuggestionsCapitalized(List<Word> suggestions) {		
		...
		for (Word word: suggestions) {			
			...
			stringBuffer.setCharAt(0, Character.toUpperCase(suggestion
					.charAt(0)));
			Word newWord = new Word(stringBuffer.toString(), word.getCost());
			suggestions.set(i, newWord);
			i++;
		}
		return suggestions;
	}
