...
public class Segment extends StructureItem 	{
	...
	public boolean read (Parser.Context context) {
		...
		if (!isSegmentStarting (context))
		{
			...
			if( context.getParticle().mMinOccurs > 0)
			{
				...
				if( sSeg.length() > 0 )
				{
					...
					else if( !context.getParser().MessageSegments.contains( sSeg ) )
					{
						context.handleError(
			...
			context.getParser().getEDIKind() == EDISettings.EDIFixed )
        {
			if( context.getParser().getEDIKind() != EDISettings.EDIFixed )
			{
				...
				if ( !(mName.equals("MSH") && context.getParser().getEDIKind() == EDISettings.EDIHL7))
		...
		if (mName.equals("ISA") &&
            context.getParser().getEDIKind() == EDISettings.EDIX12)  // X12 ISA segment defines the segment terminator here
		{
	...
	public void write (Writer writer, ITextNode node) throws IOException {
        ...
        if( !(mName.equals("MSH") && writer.getEDIKind() == EDISettings.EDIHL7))
