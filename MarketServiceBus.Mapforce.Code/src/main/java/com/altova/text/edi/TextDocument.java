...
public abstract class TextDocument extends Parser {
	...
	{
		ITextNode root = mGenerator.getRootNode();

		if (getSettings().getAutoCompleteData()) {
			DataCompletion datacompletion = null;

			switch (getEDIKind()) {
			case EDISettings.EDIFact:
				datacompletion = new EDIFactDataCompletion((EDIFactSettings) getSettings(), mStructureName);
				break;
			case EDISettings.EDIX12:
				datacompletion = new EDIX12DataCompletion((EDIX12Settings) getSettings(), mStructureName);
				break;
            case EDISettings.EDIHL7:
                datacompletion = new EDIHL7DataCompletion((EDIHL7Settings) getSettings(), mStructureName);
                break;
			case EDISettings.EDIFixed:
			    datacompletion = new EDIFixedDataCompletion((EDIFixedSettings) getSettings(), mStructureName);
                break;
			}
			if( datacompletion != null)
				datacompletion.completeData(root, mRootParticle);
			else
				System.out.println( "No data completion will be performed");
		}

		if (EDISettings.EDIFact == getEDIKind()) {
		...
		Writer ediWriter = new Writer(writer, getSettings(), getErrorSettings() );
