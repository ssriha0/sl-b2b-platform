...
public class XLSXFileWriter 
{
	...
	public void run() throws Exception
	{
		...
		ListIterator it = resultWorksheets.listIterator();
		while (it.hasNext())
		{
	...
	private void makeWorksheets(ArrayList resultWorksheets) throws Exception
	{
		...
		for (Element worksheet=XmlTreeOperations.getFirstElementChild(workbook); worksheet != null; worksheet = XmlTreeOperations.getNextElementSibling(worksheet))
		{
			if (worksheet.getLocalName().equals("Worksheet"))
			{
				...
				ListIterator itResultWorksheet = resultWorksheets.listIterator();
				...
				for (Element row=XmlTreeOperations.getFirstElementChild(worksheet); row != null; row = XmlTreeOperations.getNextElementSibling(row))
				{
					...
					if (row.getLocalName().equals("Row"))
					{
						...
						ArrayList lastRow = (ArrayList) resultWorksheet.matrix.get(lastRowSeen);
						if (lastRow == null)
	...
	private void makeCells(Element row, ArrayList resultRow, String rowIndex) throws Exception
	{
		int lastColSeen = 0;
	...
	private void createResultSheetContentsAndClearMatrices(ArrayList resultWorksheets) throws Exception
	{
		int workSheet = 0;
		...
		while(itResultWorksheet.hasNext())
		{
			...
			{
				ArrayList cells = (ArrayList) resultWorksheet.matrix.get(i);
	...
	private void makeWorkbookRels(ArrayList resultWorksheets) throws Exception
	{
		...
		int n = 1;
		ListIterator itR = resultWorksheets.listIterator();
		while (itR.hasNext())
		{
	...
	private void makeWorkbookDefinition(ArrayList resultWorksheets) throws Exception
	{
	...
	private void makeRelsContentTypes(ArrayList resultWorksheets) throws Exception
	{
	...
	{
		public ResultWorksheet(String sheetName, ArrayList matrix) {this.sheetName=sheetName; this.filename=null; this.matrix = matrix; document = null;}
