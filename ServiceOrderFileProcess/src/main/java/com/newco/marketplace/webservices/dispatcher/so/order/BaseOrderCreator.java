...
public abstract class BaseOrderCreator extends ABaseRequestDispatcher {
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 541991904457386849L;
	private static final Logger logger = Logger.getLogger(BaseOrderCreator.class.getName());
	/**
	 ...
	 * @return
	 */
	protected WSProcessResponse processResponse(WSProcessResponse response) {
	 ...
	 * @return
	 */
	protected WSProcessResponse processSecurityResponseError(WSProcessResponse serviceResponse) {
	...
	protected WSProcessResponse processBusinessError(WSProcessResponse response, Exception ex, String errCode, String errMsg, boolean warningOnly, String processStatus) {
		if (ex != null) {
	...
	protected void processBusinessResponse(String slSOId, WSProcessResponse response, ProcessResponse busProcessResponse) {
	...
	public ServiceOrderResponse closeServiceOrder(CloseServiceOrderRequest request) {
		...
		ServiceOrderBO businessObject = (ServiceOrderBO) this.getBusinessBeanFacility(SERVICE_ORDER_BUSINESS_OBJECT_REFERENCE);
	...
	protected CreateDraftResponse processOMSCreateDraft(CreateDraftRequest request) {
		return processOMSCreateDraft(request, true);
	}

	protected CreateDraftResponse processOMSCreateDraft(CreateDraftRequest request, boolean buyerAuthRequired) {
...
//		SL-10009 THIS WILL HELP IF SOMETHING FAILS
		serviceResponse.setClientServiceOrderId(request.getOrderNumber());

		if (buyerAuthRequired) {
			// WebService Authentication
			ABaseResponse securityResponse = doWSServiceAccessAuthenication(request);
			if (securityResponse.isError()) {
...
//				SL-10009 THIS WILL HELP IF SOMETHING FAILS
				serviceResponse.setClientServiceOrderId(externalServiceOrderID);
			} catch (Exception ex) {
			...
			try {
				...
				/*this should have been done in part of refactoring OrderDispatchRequestor , should be in buyer specific class*/
				if(securityContext.getClientId()!= null && securityContext.getClientId().intValue() == 3){
					subStatus = new Integer(OrderConstants.READY_FOR_POSTING_SUBSTATUS);
				}
...
//								TODO need to figure out what to do.
						...
						// BELWO PART IS DISABLED AS WE DON"T WANT TO AUTO ROUTE IN INJECTION PROCESS
	...
	protected void repriceSoBasedOnConditionalPrices(ServiceOrder so, CreateDraftRequest request, CreateDraftResponse serviceResponse, Integer ruleId) {
	...
	protected List<CreateTaskReponse> repriceOrder(ServiceOrder so, Map<String, BigDecimal> repricedItems, CreateDraftRequest request) {
		...
		for (String sku : repricedItems.keySet()) {
			...
			for (ServiceOrderTask serviceOrderTask : tasks) {
				...
				if(skuIndex  > 0){			
					if(taskName.substring(0, skuIndex).equalsIgnoreCase(sku) && null != serviceOrderTask.getPrice()){						
	 ...
	 * @return
	 */
	protected Double calculateNewSpendLimit(CreateDraftRequest request, Map<String, BigDecimal> repricedItems) {
		double total = 0d;
			...
			if (jobCode != null) {
				if (!processedSkus.contains(jobCode))  {
					if (repricedItems.containsKey(jobCode)) {
						...
						if(task.getLaborPrice() == null) {
							logger.warn("Setting labor price to 0.00 for task: "+task.toString());
							task.setLaborPrice(0.0);
						}
	 ...
	 */
	protected void groupTheOrder(CreateDraftRequest request, ServiceOrder so, TierRouteController trController, SecurityContext securityContext) throws com.newco.marketplace.exception.BusinessServiceException, BusinessServiceException {
		IRouteOrderGroupBO routeOrderGrpObj = (IRouteOrderGroupBO) this.getBusinessBeanFacility(ROUTE_ORDER_GROUP_BUSINESS_OBJECT);
	...
	protected String addCustomReferences(CreateDraftRequest request, ServiceOrder so, String externalServiceOrderID) throws Exception {
	...
	protected boolean isPrimaryCategoryGood(Integer mainCategoryId) throws Exception {
	...
	protected DocumentVO getDocumentVo(Integer buyerId, String docName) throws BusinessServiceException {
		DocumentVO documentVO = null;
	...
	protected List<BuyerReferenceVO> getBuyerReferenceTypes(Integer buyerId) throws DataServiceException {
	...
	protected boolean validateFeature(Integer buyerId, String feature) {
	...
	protected Account getAccountDetails(Integer companyId) {
	 ...
	 * @return
	 */
	protected CreateDraftResponse processException(CreateDraftResponse serviceResponse, ProcessResponse busProcessResponse, Exception coreBSEx) {
	...
	protected UpdatePartResponse savePart(CreatePartRequest request, UpdatePartResponse serviceResponse) throws Exception {
	...
	public String validateAServiceOrder(String resourceId, String soId) {
		...
		try {
			...
			if (valid) {
				serviceResponse = WSConstants.WSProcessStatus.VALID;
			} else {
	...
	public String validateServiceOrdersString(String techid, String last6digits) {
		...
		if (validServiceOrders != null) {
			if (validServiceOrders.length == 1) {
				tokenizedServiceOrders = validServiceOrders[0];
			} else {
	...
	protected void saveDocumentsToServiceOrder(List<Document> documents, Integer buyerId, String serviceOrderId, SecurityContext securityContext, WSProcessResponse serviceResponse) {
		IDocumentBO documentBO = (IDocumentBO) this.getBusinessBeanFacility(DOCUMENT_BUSINESS_OBJECT);
	...
	protected String groupOrder(ServiceOrder so, String status) {
