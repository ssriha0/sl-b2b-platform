package com.newco.marketplace.webservices.validation.so.route;
...
public class RouteValidator implements IValidator, OrderConstants{
	...
	public ValidatorResponse validate(ParamMap pMap) throws Exception {
		...
		//Why not just use ints?
		if( !(so.getWfStateId().equals(Integer.valueOf(OrderConstants.DRAFT_STATUS))))
		{
		...
		/*if(so.getLocTypeInd() == null)
			vr.setMessage(OrderConstants.SERVICE_LOCATION_TYPE_REQUIRED);*/
