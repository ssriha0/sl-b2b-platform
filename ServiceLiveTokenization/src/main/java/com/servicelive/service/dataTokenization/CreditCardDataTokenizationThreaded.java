 ...
 */
public class CreditCardDataTokenizationThreaded implements Runnable {
    ...
    long thredOffSet;
    long threadEndOffset;
    String slstoreNo;

	public CreditCardDataTokenizationThreaded(Connection connectionSchemaSupplierProd,String tokenizerUrl,String tokenizerHeader,String encryptionKey,boolean additionalData,String slStoreNo,long threadCount,long threadend) {
		this.connectionSchemaSupplierProd = connectionSchemaSupplierProd;
        this.tokenizerUrl=tokenizerUrl;
        this.tokenizerHeader=tokenizerHeader;
        this.encryptionKey=encryptionKey;
        this.additionalData=additionalData;
        this.thredOffSet = threadCount;
        this.threadEndOffset= threadend;
        this.slstoreNo = slStoreNo;
	}

	public void run() {
		Log.writeLog(Level.INFO, "Inside CreditCardDataTokenization thread..."+ Thread.currentThread().getName());
		PreparedStatement statement = null;
		try {
				offset = this.thredOffSet;
				...
				do {
					...
					statement.setLong(2,this.threadEndOffset);
					statement.setInt(3, limit);
	                resultSet = statement.executeQuery();
					// for each,
					resultListSize = tokenizeCreditCardData(resultSet); 
	                //offset = offset + limit;
					Log.writeLog(Level.INFO,"CreditCardDataTokenization : Fetch size : "+ resultListSize +" : Offset : "+ offset);
				}while (resultListSize == limit);
				Log.writeLog(Level.INFO,"Exiting CreditCardDataTokenization thread...");
			} catch (Exception e) {
			Log.writeLog(Level.SEVERE,"Exiting CreditCardDataTokenization thread with exception..."+ e);
			e.printStackTrace();
		}
	}
	...
	private int tokenizeCreditCardData(ResultSet resultSet) throws Exception {
		...
		while (resultSet.next()) {
			if (null != resultSet.getString(2)) {
				...
				if(null!=decryptedAccountNo){
			        //invoke the tokenize webservice for each credit card number
					try{
						...
						Log.writeLog(Level.SEVERE,"Exception while invoking credit card tokenization id : "+ resultSet.getString(1) + " -****** ERROR *****-");
						error=e.getMessage();
    				...
    				// result should be error if token or masked account number is null
    				   ...
    				   updateStatement.setString(5, resultSet.getString(2)); 
    					...
    					isRowUpdated = true; 
                 		...
                 		Log.writeLog(Level.SEVERE,"Could not convert row in CreditCardDataEncryption with tokenization id : "+ resultSet.getString(1) + " -****** ERROR *****-");
					//throw e;
				}
			}
		...
		if (isRowUpdated) {
			// Added for batch execution
			int [] updatedCount = updateStatement.executeBatch();
			Log.writeLog(Level.INFO,"Updated tokenized credit card in credit_card_tokenization for  "+ updatedCount.length + "records");
		}
		return rows;
	}
	...
	private String decryptAccountAndRoutingNo(String accountNo) {
		...
		cryptographyVO.setEncryptionKey(this.encryptionKey);
	...
	public long getThredOffSet() {
		return thredOffSet;
	}

	public void setThredOffSet(long thredOffSet) {
		this.thredOffSet = thredOffSet;
	}
