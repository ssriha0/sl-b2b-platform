 ...
 * 
 */
public class ServiceOrderWFManager implements IServiceOrderWFManager {
	...
	ServiceOrderWFManager(ExecutionService es) {
		executionService=es;
	}
	
	 /**
	  * new WF instance is created when SO is saved in Draft state for the first time
	  * uses jBPM executionService API
	  */
	public String createNewWFInstance(String soId, IServiceOrderType serviceOrderType, Map<String, Object> variables) {
		Map<String,Object> pvars = new HashMap<String,Object>();
		ProcessVariableUtil.addServiceOrderId(pvars, soId);
		ProcessVariableUtil.addServiceOrderType(pvars, serviceOrderType.getSoTypeName());
		if (variables!=null) {
			pvars.putAll(variables);
		}
		logger.debug("creating WF instance. process definition=" + serviceOrderType.getWFProcessDefinitionName() + " order#="+soId);
		ProcessInstance pi = executionService.startProcessInstanceByKey(serviceOrderType.getWFProcessDefinitionName(), pvars, soId);
		return pi.getId();
	}
