...
public class AcceptSignal extends Signal {
    ...
    protected void update(SOElement soe, ServiceOrder so)  {
        ...
        so.setAcceptedProviderResourceId(tmpSo.getAcceptedProviderResourceId());
        ...
        so.getSOWorkflowControls().setMethodOfAcceptance("MANUAL");
    ...
    protected List<String> validate(SOElement soe, ServiceOrder soTarget) {
        ...
        if(srcServiceOrder.getVendorResourceContact() == null){
            returnVal.add("Please specify Provider Contact information.");
        } else {
        	returnVal.addAll(srcServiceOrder.getVendorResourceContact().validate());
        }
        if (srcServiceOrder.getVendorResourceLocation() == null){
        	returnVal.add("Please specify Provider Location information.");
        } else {
        	returnVal.addAll(srcServiceOrder.getVendorResourceLocation().validate());
        }
        if (srcServiceOrder.getAcceptedProviderId() == null){
        	returnVal.add("Please specify Accepted Provider Id");
        }
        return returnVal;
    }
}
