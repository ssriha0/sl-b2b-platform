...
public class BuyerAgreeCmd extends SOCommand {
	...
	public void execute(Map<String, Object> processVariables) {
		...
		if (null != pendingCancelProviderHistory
				&& null != pendingCancelProviderHistory.getPrice()) {
			additionalAmount = pendingCancelProviderHistory.getPrice()-price;
		}
		...
		so.setSpendLimitLabor(new BigDecimal(pendingCancelProviderHistory.getPrice()));
		so.setSpendLimitParts(new BigDecimal(0.0));
		so.setFinalPriceLabor(new BigDecimal(pendingCancelProviderHistory.getPrice()));
		so.setFinalPriceParts(new BigDecimal(0.0));
		so.getPrice().setDiscountedSpendLimitLabor(new BigDecimal(pendingCancelProviderHistory.getPrice()).setScale(2, RoundingMode.HALF_UP));
