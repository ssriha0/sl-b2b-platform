...
public class CommonCondAutoRouteBasedEnhancer extends AbstractOrderEnhancer {
    ...
    public void enhanceOrder(ServiceOrder serviceOrder, OrderEnhancementContext orderEnhancementContext) {
    	logger.info("Entered CommonCondAutoRouteBasedEnhancer...");
        BuyerFeatureLookup buyerFeatureLookup = quickLookupCollection.getBuyerFeatureLookup();
        logger.info("isCreateWithOutTasks()::"+serviceOrder.isCreateWithOutTasks());
        if ((!buyerFeatureLookup.isActiveFeatureAssociatedWithBuyer(BuyerFeatureSetEnum.CONDITIONAL_ROUTE, serviceOrder.getBuyerId()))||(serviceOrder.isCreateWithOutTasks())) {
        	logger.info("inside return");
            return;
        }
        logger.info("outside return");
        
        CondRoutingRuleVO condRoutingRuleVO = new CondRoutingRuleVO();
        condRoutingRuleVO.setBuyerId(serviceOrder.getBuyerId().intValue());
        ...
        logger.info("Calling getConditionalRoutingRuleId->");
    ...
    private void repriceOrderUsingSkuPriceList(ServiceOrder serviceOrder, List<NameValuePair> skuPriceList) {
        ...
        BigDecimal totalPermitPrice = new BigDecimal("0.00");
        for (SOTask soTask : serviceOrder.getTasks()) {
        	...
        	//SL-20527 : Code changes to fix the mismatch in total permit price during update file injection for CAR orders
            if(null != soTask && null!= soTask.getTaskType() && (soTask.getTaskType()== 1)){
        ...
        logger.info("Total Price of active tasks:" + totalPrice);
        logger.info("Total Price of permit tasks:" + totalPermitPrice);
        PricingUtil.setTotalRetailPrice(serviceOrder, totalPrice);
        logger.info("permit tasks for :"+serviceOrder.getSoId()+" is before update" + serviceOrder.getTotalPermitPrice());
        serviceOrder.setTotalPermitPrice(totalPermitPrice);
        logger.info("permit tasks for :"+serviceOrder.getSoId()+" is after update" + serviceOrder.getTotalPermitPrice());
    ...
    private void updatePriceForFirstTaskWithSku(SOTask task, List<NameValuePair> skuPriceList){
   	 for (NameValuePair skuPricePair : skuPriceList) {
        	...
        	logger.info("sku value"+sku);
        	logger.info("task.getExternalSku() value"+task.getExternalSku());
            if (task.getExternalSku() != null) {
