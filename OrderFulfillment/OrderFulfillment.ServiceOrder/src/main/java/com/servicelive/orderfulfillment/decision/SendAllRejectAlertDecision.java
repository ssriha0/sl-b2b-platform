...
public class SendAllRejectAlertDecision extends AbstractServiceOrderDecision {
    /**
	 * 
	 */
	...
	public String decide(OpenExecution execution) {
        ...
        return allRoutedProvidersRejected(serviceOrder.getRoutedResources()) ? "sendAllRejectAlert" : "noAlert";
    }

    private boolean allRoutedProvidersRejected(List<RoutedProvider> routedProviderList) {
        if (routedProviderList.size()==0) return false;

        for (RoutedProvider routedProvider : routedProviderList) {
            if (routedProvider.getProviderResponse() != ProviderResponseType.REJECTED) {
                return false;
            }
        }
