...
import com.newco.marketplace.api.beans.firmDetail.Firms;
...
/**
 * This class would act as a mapper class for GetFirmDetailsService
 * @author neenu_manoharan
 *
 */
public class FirmDetailsMapper {
	private Logger logger = Logger.getLogger(FirmDetailsMapper.class);
	 ...
	 * @return
	 */
	public GetFirmDetailsResponse mapFirmDetailsResponse(FirmDetailsResponseVO firmDetailsResponseVO, FirmDetailRequestVO firmDetailRequestVO) {
		...
		if(null != firmDetailRequestVO){
			for(String firmId:firmDetailRequestVO.getFirmId()){
				...
				firmDetails = mapBasicFirmResponse(firmDetailsResponseVO,firmId);
	 ...
	 * @return
	 */
	private FirmDetails mapBasicFirmResponse(FirmDetailsResponseVO firmDetailsResponseVO,String firmId){
		...
		if(null != detailsVO){
			...
			if(null != detailsVO.getMinHourlyRate()){
				if(null != detailsVO.getMaxHourlyRate()){
					firmDetails.setHourlyRate("$"+dfrmt.format(detailsVO.getMinHourlyRate())+" - $"+dfrmt.format(detailsVO.getMaxHourlyRate()));
				}
				else{
					firmDetails.setHourlyRate("$"+dfrmt.format(detailsVO.getMinHourlyRate()));
				}
			}
