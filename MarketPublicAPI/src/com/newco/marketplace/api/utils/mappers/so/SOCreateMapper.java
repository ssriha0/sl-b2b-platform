...
public class SOCreateMapper {
	...
	private ArrayList<LuBuyerRefVO> customRefList = null;
	 ...
	 * @return serviceOrder
	 */
	public HashMap<String, Object> mapServiceOrder(ServiceOrderBean request,
		...
		buyerResource.setContactId(securityContext.getVendBuyerResId());
		...
		serviceOrder.setBuyerResourceId(securityContext.getVendBuyerResId());
	...
	private HashMap<String, Object> mapTemplateName(Integer companyId,
			ServiceOrderBean request, ServiceOrder serviceOrder) {
		...
		if (null != templateDto) {
			...
			Integer buyerResId=0;
			if (null!=request.getContacts()&&null != request.getContacts().getBuyerSupportResID()
					&& (!"0".equals(request.getContacts().getBuyerSupportResID()))) {

					buyerResId = new Integer(request.getContacts()
			...
			if(null!=templateDto.getAltBuyerContactId()&&buyerResId.equals(new Integer(0))){
				serviceOrder.setBuyerContactId(templateDto.getAltBuyerContactId());
			}
			if(null!=templateDto.getAltBuyerContact()&&buyerResId.equals(new Integer(0))){
				serviceOrder.setBuyerResourceId(templateDto.getAltBuyerContact().getResourceId());
			}
	...
	public SOCreateResponse createSOResponseMapping(
		...
		if (!orderStatus.getSoId().equalsIgnoreCase(
			...
			result.setMessage(CommonUtility
					.getMessage(PublicAPIConstant.DRAFT_CREATED_RESULT_CODE));
			...
			result.setMessage(CommonUtility
					.getMessage(PublicAPIConstant.DRAFT_NOTCREATED_ERROR_CODE));
