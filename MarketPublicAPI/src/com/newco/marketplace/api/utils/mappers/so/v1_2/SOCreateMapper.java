...
public class SOCreateMapper {
	 ...
	 * @return serviceOrder
	 */
	public HashMap<String, Object> mapServiceOrder(SOCreateRequest createRequest, SecurityContext securityContext) throws DataException {
		...
		if (securityContext.getRoles() == null || securityContext.getRoles().getContactId() == null) {
			logger.error("The roles or Contact Id is null in the " + "Security Context");
			throw new NullPointerException(CommonUtility.getMessage(PublicAPIConstant.CONTACT_ERROR_CODE));
		}
		...
		serviceOrder.setCreatedDate(new Timestamp(DateUtils.getCurrentDate().getTime()));
		...
		mapServiceOrderSchedule(serviceOrder, request.getScheduleServiceSlot(), soTitlesMap);
		...
		pickUpContact = mapServiceOrderContacts(serviceOrder, request.getContacts(), soTitlesMap);
		if (!soTitlesMap.isEmpty()) {
			return soTitlesMap;
		}
		...
		mapServiceOrderCustomRef(serviceOrder, request.getCustomReferences(), securityContext.getCompanyId());
		...
		if (null != request.getTemplate()) {
			soTitlesMap = mapTemplateName(securityContext.getCompanyId(), request.getTemplate(), serviceOrder);
		} else {
	 ...
	 */
	private HashMap<String, Object> mapTemplateName(Integer companyId, String templateName, ServiceOrder serviceOrder) {

		HashMap<String, Object> soTitlesMap = new HashMap<String, Object>();
		List<String> fileTitles = null;
		BuyerSOTemplateDTO templateDto = buyerSOTemplateBO.loadBuyerSOTemplate(companyId, templateName);
		if (null != templateDto) {
			...
			if (null != templateDto.getTitle()) {
				serviceOrder.setSowTitle(templateDto.getTitle() + "-"
						+ (serviceOrder.getSowTitle() != null ? serviceOrder.getSowTitle() : ""));
			}
			...
			if (null != templateDto.getSpecialInstructions()) {
				serviceOrder.setProviderInstructions(serviceOrder.getProviderInstructions() + templateDto.getSpecialInstructions());
			}

			if (null != templateDto.getConfirmServiceTime()) {
				serviceOrder.setProviderServiceConfirmInd(templateDto.getConfirmServiceTime());
			}
			...
			BuyerSOTemplateContactDTO contactDto = templateDto.getAltBuyerContact();
	...
	private ServiceOrder mapDocumentLogotoSO(ServiceOrder serviceorder, BuyerSOTemplateDTO templateDto) {
		...
		try {
			documentVO = documentBO.retrieveDocumentByTitleAndEntityID(Constants.DocumentTypes.BUYER, templateDto.getDocumentLogo(),
	 ...
	 */
	private void mapScopeOfWork(ServiceOrder serviceOrder, ScopeOfWork scopeOfWork, Integer buyerId, HashMap<String, Object> soTitlesMap) {
		...
		if (null != scopeOfWork.getSkus() && null != scopeOfWork.getSkus().getSkuList()) {
			...
			if (!invalidSkus.isEmpty()) {
				soTitlesMap.put(PublicAPIConstant.ERROR_KEY, CommonUtility.getMessage(PublicAPIConstant.SKU_INVALID_OR_NO_TASK_MAPPING)
	...
	private void mapServiceLocation(ServiceOrder serviceOrder, Location location) {
		...
		if (PublicAPIConstant.LOCATION_TYPE_COMMERCIAL.equalsIgnoreCase(locationType)) {
			serviceLocation.setLocnClassId(1);
		} else if (PublicAPIConstant.LOCATION_TYPE_RESIDENTIAL.equalsIgnoreCase(locationType)) {
	 ...
	 * @return void
	 */
	private void mapServiceOrderSchedule(ServiceOrder serviceOrder, ScheduleServiceSlot scheduleServiceSlot,
		...
		if (null != scheduleServiceSlot.getServiceDatetimeSlots()
			...
			serviceOrder.setServiceDatetimeSlots(new ArrayList<ServiceDatetimeSlot>());
			...
			for (com.newco.marketplace.dto.vo.dateTimeSlots.ServiceDatetimeSlot serviceDatetimeSlotTemp : scheduleServiceSlot
				...
				serviceDatetimeSlot = new ServiceDatetimeSlot();
				...
				serviceOrder.getServiceDatetimeSlots().add(serviceDatetimeSlot);
			}

			logger.info("total slots added : " + serviceOrder.getServiceDatetimeSlots().size());
	 ...
	 * @return void
	 */
	private void validateSODates(ServiceOrder serviceOrder, HashMap<String, Object> soTitlesMap) {
		...
		Timestamp newStartTimeCombined = new Timestamp(TimeUtils.combineDateAndTime(serviceOrder.getServiceDate1(),
				serviceOrder.getServiceTimeStart(), serviceOrder.getServiceLocationTimeZone()).getTime());
		if (null != serviceOrder.getServiceDate2()) {
			Timestamp newEndTimeCombined = new Timestamp(TimeUtils.combineDateAndTime(serviceOrder.getServiceDate2(),
					serviceOrder.getServiceTimeEnd(), serviceOrder.getServiceLocationTimeZone()).getTime());

			if (serviceOrder.getServiceDateTypeId() == 2) {
				if (newStartTimeCombined.compareTo(newEndTimeCombined) >= 0) {

					soTitlesMap.put(PublicAPIConstant.ERROR_KEY, CommonUtility.getMessage(PublicAPIConstant.STARTDATE_ENDDATE));
					return;
		...
		if (newStartTimeCombined.compareTo(now) < 0) {
			soTitlesMap.put(PublicAPIConstant.ERROR_KEY, CommonUtility.getMessage(PublicAPIConstant.STARTDATE_IMPROPER));
			return;
		}
	...
	private void mapServiceOrderPricing(ServiceOrder serviceOrder, Pricing pricing) {
		...
		if (null != pricing.getLaborSpendLimit() && (!"".equals(pricing.getLaborSpendLimit()))
			...
			Double tempLabourSpendLimit = Double.parseDouble(pricing.getLaborSpendLimit());
		...
		if (null != pricing.getPartsSpendLimit() && (!"".equals(pricing.getPartsSpendLimit()))
			...
			Double tempPartsSpendLimit = Double.parseDouble(pricing.getPartsSpendLimit());
	 ...
	 * @return void
	 */
	private Contact mapServiceOrderContacts(ServiceOrder serviceOrder, Contacts contacts, HashMap<String, Object> soTitlesMap) {
			...
			try {
				...
				if (contactLocationVO != null && contactLocationVO.getBuyerPrimaryContact() != null) {
					...
					if (buyerContactId == 0 || null == buyerContactId) {
						soTitlesMap
								.put(PublicAPIConstant.ERROR_KEY, CommonUtility.getMessage(PublicAPIConstant.BUYER_RESOURCEID_NOT_VALID));
					}
				} else {
					soTitlesMap.put(PublicAPIConstant.ERROR_KEY, CommonUtility.getMessage(PublicAPIConstant.BUYER_RESOURCEID_NOT_VALID));
				}
			} catch (Exception e) {
		...
		if ((null != contacts && null != contacts.getContactList()) && soTitlesMap.isEmpty()) {
			...
			while (contactList.hasNext()) {
				...
				if (contactRequst.getContactLocnType().equalsIgnoreCase(PublicAPIConstant.SERVICE)) {
					serviceOrder.setServiceContact(mapContactDetails(contactRequst));
				} else if (contactRequst.getContactLocnType().equalsIgnoreCase(PublicAPIConstant.END_USER)) {
					serviceOrder.setEndUserContact(mapContactDetails(contactRequst));
	 ...
	 * @return void
	 */
	private void mapServiceOrderDocuments(ServiceOrder serviceOrder, Attachments attachments) {
	 ...
	 * @return void
	 */
	private HashMap<String, Object> mapServiceOrderParts(ServiceOrder serviceOrder, Parts parts, Contact pickUpContact,
			HashMap<String, Object> soTitlesMap) {
			...
			while (partsList.hasNext()) {
				...
				if (partVo.isProviderBringPartInd()) {
					if (reqPart.getPickupLocation() != null) {
						...
						if (PublicAPIConstant.LOCATION_TYPE_COMMERCIAL.equalsIgnoreCase(pickupLocation.getLocationType())) {
							soPickupLocation.setLocnClassId(1);
						} else if (PublicAPIConstant.LOCATION_TYPE_RESIDENTIAL.equalsIgnoreCase(pickupLocation.getLocationType())) {
							soPickupLocation.setLocnClassId(2);
						}
						soPickupLocation.setCreatedDate(new Timestamp(DateUtils.getCurrentDate().getTime()));
						soPickupLocation.setLocName(pickupLocation.getLocationName());
	 ...
	 * @return void
	 */
	private void mapServiceOrderCustomRef(ServiceOrder serviceOrder, CustomReferences customReferences, Integer ownerId) {
		...
		if (null != customReferences && null != customReferences.getCustomRefList()) {
			Iterator<CustomReference> customRefsVoList = customReferences.getCustomRefList().iterator();
			while (customRefsVoList.hasNext()) {
				CustomReference customRef = (CustomReference) customRefsVoList.next();
				...
				customRefVO.setRefTypeId(getCustomRefTypeId(ownerId + "", customRef.getName()));
	...
	private Integer getPhoneTypeId(String phoneType) {
		...
		try {
			...
			logger.error("Data Exception Occurred while getting  PhoneTypeId " + e);
		}
		return null;
	}
	 ...
	 * @return contact
	 */
	private Contact mapContactDetails(com.newco.marketplace.api.beans.so.Contact requestContact) {
	 ...
	 */
	public SOCreateResponse createSOResponseMapping(ProcessResponse processResponse, List<String> invalidDocumentList) {
