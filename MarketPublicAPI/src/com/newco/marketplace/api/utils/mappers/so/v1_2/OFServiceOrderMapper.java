package com.newco.marketplace.api.utils.mappers.so.v1_2;
...
import com.newco.marketplace.api.beans.so.create.v1_2.SOCreateRequest;
import com.newco.marketplace.api.beans.so.create.v1_2.ServiceOrderBean;
import com.newco.marketplace.api.beans.so.edit.SOEditResponse;
import com.newco.marketplace.api.beans.so.update.SOSchedule;
...
public class OFServiceOrderMapper extends OFMapper {
	 ...
	 * @return serviceOrder
	 */
	public CreateOrderRequest createServiceOrder(SOCreateRequest createRequest, SecurityContext securityContext)
			throws BusinessServiceException, ValidationException {
		...
		/*
		 * SL-16834 :If the buyer resource id is present in the request, set the
		 * buyer resource id and the contact id of the buyer resource while
		 * creating the service order.
		 */
		if (null != createRequest.getIdentification() && null != createRequest.getIdentification().getId()
				&& PublicAPIConstant.BUYER_RESOURCE_ID.equalsIgnoreCase(createRequest.getIdentification().getType())) {
	...
	public OrderFulfillmentRequest editServiceOrder(String soId, ServiceOrderBean request, SecurityContext securityContext)
	...
	protected void mapServiceOrder(ServiceOrder serviceOrder, ServiceOrderBean request, SecurityContext securityContext)
			throws ValidationException {
		...
		validateZipAndState(PublicAPIConstant.SERVICE, request.getServiceLocation().getZip(), request.getServiceLocation().getState(),
				validationErrors);
		...
		// R12_1
		// SL-20177 : Invalid zip code validation was failing due to empty time
		// zone
		...
		if (null == request.getScheduleServiceSlot()) {
			...
			ServiceDatetimeSlot serviceSlot = new ServiceDatetimeSlot();
			serviceSlot.setServiceStartDate(request.getSchedule().getServiceDateTime1());
			serviceSlot.setServiceEndDate(request.getSchedule().getServiceDateTime2());

			List<ServiceDatetimeSlot> slots = new ArrayList<ServiceDatetimeSlot>();
			slots.add(serviceSlot);

			ServiceDateTimeSlots servideSlots = new ServiceDateTimeSlots();
			servideSlots.setServiceDatetimeSlot(slots);

			ScheduleServiceSlot scheduleServiceSlot = new ScheduleServiceSlot();
			scheduleServiceSlot.setScheduleType((request.getSchedule().getScheduleType()));
			scheduleServiceSlot.setConfirmWithCustomer(request.getSchedule().getConfirmWithCustomer());
			scheduleServiceSlot.setReschedule(request.getSchedule().getReschedule());
			scheduleServiceSlot.setServiceLocationTimezone(request.getSchedule().getServiceLocationTimezone());
			scheduleServiceSlot.setServiceDatetimeSlots(servideSlots);
			request.setScheduleServiceSlot(scheduleServiceSlot);
	...
	private void mapBuyerInformation(CreateOrderRequest orderRequest, ServiceOrder serviceOrder, SecurityContext securityContext,
			Identification identification) throws BusinessServiceException {

		if (securityContext.getCompanyId() == null) {
			logger.error("The company ID is null in the Security Context");
			throw new NullPointerException(CommonUtility.getMessage(PublicAPIConstant.COMPANY_ERROR_CODE));
		}
		...
		if (securityContext.getVendBuyerResId() == null) {
			logger.error("The VendBuyerResource Id is null " + "in the Security Context");
			throw new NullPointerException(CommonUtility.getMessage(PublicAPIConstant.VENDOR_BUYERRESOURCE_ERROR_CODE));
		}
		...
		if (null != identification && null != identification.getId()
			...
			// SL-16834 :Get the contact id of the buyer resource and set the
			// id.
			ContactLocationVO contactLocationVO = new ContactLocationVO();
			contactLocationVO = buyerBO.getBuyerResContactLocationVO(securityContext.getCompanyId(), identification.getId());
	...
	private void mapBuyerInformationForEdit(ServiceOrder serviceOrder, SecurityContext securityContext) throws BusinessServiceException {
	 ...
	 */
	// private ServiceOrder mapDocumentLogotoSO(ServiceOrder
	// serviceorder,BuyerSOTemplateDTO templateDto) {
	//
	...
	private void mapScopeOfWork(ServiceOrder serviceOrder, ScopeOfWork scopeOfWork, Long buyerId, List<String> validationErrors) {
		...
		if (null != scopeOfWork.getSkus() && null != scopeOfWork.getSkus().getSkuList()) {// orders
																							// from
																							// BIP
			...
			if (null != reqTasks && null != reqTasks.getTaskList()) {
				...
				/*
				 * Set the first task as primary task. Set whether tasks are
				 * present
				 */
				if (tasks.size() > 0) {
					tasks.get(0).setPrimaryTask(true);
	...
	private void mapServiceOrderSchedule(ServiceOrder serviceOrder,
		...
		com.servicelive.orderfulfillment.domain.SOSchedule soSchedule = new com.servicelive.orderfulfillment.domain.SOSchedule();
	...
	private void mapServiceOrderPricing(ServiceOrder serviceOrder, Pricing pricing, List<String> validationErrors) {
		...
		if (null != pricing.getLaborSpendLimit() && (!"".equals(pricing.getLaborSpendLimit()))
				&& (!"0".equals(pricing.getLaborSpendLimit())) && (!"0.0".equals(pricing.getLaborSpendLimit()))) {
		...
		if (null != pricing.getPartsSpendLimit() && (!"".equals(pricing.getPartsSpendLimit()))
				&& (!"0".equals(pricing.getPartsSpendLimit())) && (!"0.0".equals(pricing.getPartsSpendLimit()))) {
	...
	private SOPrice mapPricing(Pricing pricing, ServiceOrder so) {
		...
		BigDecimal laborTax = new BigDecimal(null != pricing.getLaborTax() && pricing.getLaborTax().trim().length() > 0 ? pricing.getLaborTax() : "0.00");
		returnVal.setTaxPercentLaborSL(laborTax);
		BigDecimal partsTax = new BigDecimal(null != pricing.getPartsTax() && pricing.getPartsTax().trim().length() > 0 ? pricing.getPartsTax() : "0.00");
		returnVal.setTaxPercentPartsSL(partsTax);
		
		BigDecimal laborDiscount = new BigDecimal(null != pricing.getLaborDiscount() && pricing.getLaborDiscount().trim().length() > 0 ? pricing.getLaborDiscount() : "0.00");
		returnVal.setDiscountPercentLaborSL(laborDiscount);
		BigDecimal partsDiscount = new BigDecimal(null != pricing.getPartsDiscount() && pricing.getPartsDiscount().trim().length() > 0 ? pricing.getPartsDiscount() : "0.00");
		returnVal.setDiscountPercentPartsSL(partsDiscount);
	...
	private SOContact mapServiceOrderContacts(ServiceOrder serviceOrder, Contacts contacts, List<String> validationErrors) {
		...
		SOContact pickUpContact = null;
		if (null != contacts.getBuyerSupportResID() && (!"0".equals(contacts.getBuyerSupportResID()))) {
			try {
				Integer buyerResId = new Integer(contacts.getBuyerSupportResID());
		...
		if ((null != contacts && null != contacts.getContactList()) && validationErrors.isEmpty()) {
			Iterator<com.newco.marketplace.api.beans.so.Contact> contactList = contacts.getContactList().iterator();

			while (contactList.hasNext()) {
				com.newco.marketplace.api.beans.so.Contact contactRequst = (com.newco.marketplace.api.beans.so.Contact) contactList.next();
				...
				if (contactRequst.getContactLocnType().equalsIgnoreCase(PublicAPIConstant.SERVICE)) {
					...
					serviceOrder.addContact(contact);
				} else if (contactRequst.getContactLocnType().equalsIgnoreCase(PublicAPIConstant.PICKUP)) {
	 ...
	 * @return void
	 */
	private List<String> mapServiceOrderParts(ServiceOrder serviceOrder, Parts parts, SOContact pickUpContact, List<String> validationErrors) {
		...
		if (null != parts && null != parts.getPartList()) {
			Iterator<com.newco.marketplace.api.beans.so.Part> partsList = parts.getPartList().iterator();
			while (partsList.hasNext()) {
				com.newco.marketplace.api.beans.so.Part reqPart = (com.newco.marketplace.api.beans.so.Part) partsList.next();
				...
				if (null != reqPart.getDimensions()) {
					...
					if (PublicAPIConstant.STANDARD_ENGLISH.equalsIgnoreCase((reqPart.getDimensions().getMeasurementType()))) {
						partVo.setMeasurementStandard(Integer.parseInt(OrderConstants.ENGLISH));
					} else if (PublicAPIConstant.STANDARD_METRIC.equalsIgnoreCase(reqPart.getDimensions().getMeasurementType())) {
						partVo.setMeasurementStandard(Integer.parseInt(OrderConstants.METRIC));
					}
				}
				...
				shippingCarrier.setCarrierId(getShippingCarrierId(reqPart.getShipCarrier()));
				returnCarrier.setCarrierName(reqPart.getReturnCarrier());
				returnCarrier.setTrackingNumber(reqPart.getReturnTracking());
				returnCarrier.setCarrierId(getShippingCarrierId(reqPart.getReturnCarrier()));
				...
				partVo.setProviderBringPartInd(Boolean.parseBoolean(reqPart.getRequiresPickup()));
				if (partVo.getProviderBringPartInd()) {
					if (reqPart.getPickupLocation() != null) {
						...
						soPickupLocation.setCreatedDate(new Timestamp(DateUtils.getCurrentDate().getTime()));
						soPickupLocation.setLocationName(pickupLocation.getLocationName());
						soPickupLocation.setStreet1(pickupLocation.getAddress1());
						soPickupLocation.setStreet2(pickupLocation.getAddress2());
		...
		if (null != customReferences && null != customReferences.getCustomRefList()) {
			for (CustomReference customRef : customReferences.getCustomRefList()) {
				...
				if (ownerId.intValue() == PublicAPIConstant.BUYER_9000.intValue()
						&& (PublicAPIConstant.SHOPIFY_ORDER_SKU_PRICE1.equalsIgnoreCase(refType)
								|| PublicAPIConstant.SHOPIFY_ORDER_SKU_PRICE2.equalsIgnoreCase(refType)
								|| PublicAPIConstant.SHOPIFY_ORDER_SKU_PRICE3.equalsIgnoreCase(refType) || PublicAPIConstant.SHOPIFY_ORDER_SKU_PRICE4
									.equalsIgnoreCase(refType)) && StringUtils.isNotBlank(customRef.getValue())) {
					customRefVO.setBuyerRefValue(customRef.getValue().replace(PublicAPIConstant.TARGET_STRING,
							PublicAPIConstant.REPLACEMENT_STRING));
				} else {
			...
			// see if the template name is not blank and add it as custom
			// reference
	...
	private Integer getServiceTypeId(Integer mainCategoryId, String serviceType) {
		...
		try {
			...
			serviceTypes = skillAssignBOImpl.getServiceTypes(mainCategoryId);
			// }
			if (null != serviceTypes) {
				for (com.newco.marketplace.vo.provider.ServiceTypesVO serviceTypesVO : serviceTypes) {
					if (serviceTypesVO.getDescription().equalsIgnoreCase(serviceType)) {
			...
			logger.error("Data Exception Occurred while getting " + " ServiceTypeId " + e);
		}
		return null;
	}
	...
	private Integer getPhoneTypeId(String phoneType) {
		...
		try {
			...
			logger.error("Data Exception Occurred while getting  PhoneTypeId " + e.getMessage(), e);
		}
		return null;
	}
	...
	private Integer getShippingCarrierId(String carrierType) {
		...
		try {
			...
			logger.error("Data Exception Occurred while Setting the Shipping " + "Carrier Details" + e);
		}
		return null;
	}
	 ...
	 * @return contact
	 */
	private SOContact mapContactDetails(com.newco.marketplace.api.beans.so.Contact requestContact) {
		...
		contact.setCreatedDate(new Timestamp(DateUtils.getCurrentDate().getTime()));
		return contact;
	}
	...
	private SOPhone mapPhoneDetails(Phone phone, int phoneTypeId) {
		...
		phoneVO.setPhoneNo(StringUtils.isEmpty(phone.getNumber()) ? null : phone.getNumber().replace("-", ""));
	...
	public SOCreateResponse createSOResponseMapping(String soId, List<String> invalidDocumentList) {
		...
		if (!orderStatus.getSoId().equalsIgnoreCase(PublicAPIConstant.INVALID_SOID)) {
			...
			if (!invalidDocumentList.isEmpty()) {
				...
				results.setError(Results.getError(errorMessage, ResultsCode.WARNING.getCode()).getError());
			}

		} else {
			results = Results.getError(ResultsCode.DRAFT_NOT_CREATED.getMessage(), ResultsCode.DRAFT_NOT_CREATED.getCode());
		}
	...
	public SOEditResponse editSOResponseMapping(OrderFulfillmentResponse ofResponse) {
		...
		if (ofResponse.isError()) {
			Results errorResults = Results.getError(ofResponse.getErrorMessage(), ResultsCode.EDIT_FAILURE.getCode());
			soEditResponse.setResults(errorResults);
		} else {
			Results results = Results.getSuccess(ResultsCode.EDIT_SUCCESSFUL.getMessage());
	...
	public SOUpdateResponse updateSOResponseMapping(OrderFulfillmentResponse ofResponse) {
		...
		if (ofResponse.isError()) {
			Results errorResults = Results.getError(ofResponse.getErrorMessage(), ResultsCode.UPDATE_FAILURE.getCode());
			soUpdateResponse.setResults(errorResults);
		} else {
			Results results = Results.getSuccess(ResultsCode.UPDATE_SUCCESSFUL.getMessage());
	...
	private Integer getCustomRefTypeId(String buyerId, String customRefType) {
		...
		try {
			...
			logger.error("Data Exception Occurred while getting " + " CustomerRefId " + "Carrier Details" + e);
		}
		return null;
	}
	 ...
	 */
	public OrderFulfillmentRequest updateServiceOrder(String svcOrdrId, SOUpdateRequest request, SecurityContext securityContext,
		...
		if (StringUtils.isNotBlank(soSubstatus)) {
			...
			if ((OrderConstants.ACCEPTED_STATUS == serviceOrder.getWfStateId().intValue())
					|| (OrderConstants.ACTIVE_STATUS == serviceOrder.getWfStateId().intValue())
					|| (OrderConstants.PROBLEM_STATUS == serviceOrder.getWfStateId().intValue())
					|| (OrderConstants.COMPLETED_STATUS == serviceOrder.getWfStateId().intValue())) {
				...
				if (!(OrderConstants.REVISIT_NEEDED_SUBSTATUS_ID.equals(serviceOrder.getWfSubStatusId())
						...
						|| OrderConstants.WORK_STARTED_SUBSTATUS_ID.equals(serviceOrder.getWfSubStatusId()) || OrderConstants.NOT_HOME_REVISIT_NEEDED_SUBSTATUS_ID
							.equals(serviceOrder.getWfSubStatusId()))) {
		...
		if (null != request.getSpendLimitIncrease()) {
			...
			if (null != serviceOrder.getSpendLimitReasonId()) {
				orderRequest.setParameter(OrderfulfillmentConstants.PVKEY_SPEND_LIMIT_REASON, serviceOrder.getSpendLimitReasonId()
						.toString());
			}
		}
		...
		ContactLocationVO contactLocationVO = new ContactLocationVO();
		contactLocationVO = buyerBO.getBuyerResContactLocationVO(securityContext.getCompanyId(), buyerResource.getResourceId());

		contact = contactLocationVO.getBuyerPrimaryContact();

		// LoginCredentialVO roles = securityContext.getRoles();
		if (null != contact && null != contact.getFirstName().trim() && null != contact.getLastName().trim()) {
		...
		if ((OrderConstants.ROUTED_STATUS == serviceOrder.getWfStateId().intValue() || OrderConstants.DRAFT_STATUS == serviceOrder
				.getWfStateId().intValue()) ) {
			
			if (null != request.getScheduleServiceSlot()) {
				updateServiceOrderDates(serviceOrder, validationErrors, request.getScheduleServiceSlot(), orderRequest);
				...
				for(ServiceDatetimeSlot serviceDatetimeSlot : request.getScheduleServiceSlot().getServiceDatetimeSlots().getServiceDatetimeSlot()){
					...
					if("1".equals(serviceDatetimeSlot.getPreferenceInd())){
						soSchedule.setServiceDateTime1(serviceDatetimeSlot.getServiceStartDate());
						soSchedule.setServiceDateTime2(serviceDatetimeSlot.getServiceEndDate());
					}
				}
				//soSchedule.setServiceDateTime1(request.getScheduleServiceSlot().getServiceDatetimeSlots().getServiceDatetimeSlot().get(0).getServiceStartDate());
				//soSchedule.setServiceDateTime2(request.getScheduleServiceSlot().getServiceDatetimeSlots().getServiceDatetimeSlot().get(0).getServiceEndDate());
				soSchedule.setScheduleType(request.getScheduleServiceSlot().getScheduleType());
				request.setSchedule(soSchedule);
			}
			
			if (null != request.getSchedule()) {
				updateServiceOrderDates(serviceOrder, validationErrors, request.getSchedule());
	 ...
	 * @throws ValidationException
	 */
	private void updateServiceOrderDates(ServiceOrder serviceOrder, List<String> validationErrors, ScheduleServiceSlot scheduleServiceSlot, OrderFulfillmentRequest orderRequest)
			throws ValidationException {
		logger.info("Mapping: ServiceOrder Schedule --->Starts");
		if (null != scheduleServiceSlot.getServiceDatetimeSlots()
				&& null != scheduleServiceSlot.getServiceDatetimeSlots().getServiceDatetimeSlot()) {
			if (null != serviceOrder.getSoServiceDatetimeSlot()) serviceOrder.getSoServiceDatetimeSlot().clear();
			SOServiceDatetimeSlot serviceDatetimeSlot = null;
			logger.info("Total time slots received : " + scheduleServiceSlot.getServiceDatetimeSlots().getServiceDatetimeSlot().size());
			for (com.newco.marketplace.dto.vo.dateTimeSlots.ServiceDatetimeSlot serviceDatetimeSlotTemp : scheduleServiceSlot
					.getServiceDatetimeSlots().getServiceDatetimeSlot()) {
			...
			orderRequest.setParameter("SO_ADDED_SERVICE_DATETIME_SLOTS","true");
		}
	}
	 ...
	 * @throws ValidationException
	 */
	private void updateServiceOrderDates(ServiceOrder serviceOrder, List<String> validationErrors,
			SOSchedule schedule) throws ValidationException {

		com.servicelive.orderfulfillment.domain.SOSchedule soSchedule = serviceOrder.getSchedule();
		if (null != schedule.getServiceDateTime1() && !schedule.getServiceDateTime1().equals("")) {

			soSchedule.setServiceDate1(DateUtils.defaultFormatStringToDate(schedule.getServiceDateTime1()));

			// setting the servicestartTime
			Date serviceStartTime = null;
			try {
				serviceStartTime = CommonUtility.sdfToDate.parse(schedule.getServiceDateTime1());
			} catch (ParseException e) {
				logger.error("Exception Occurred while setting Service " + "Start Time in OFServiceOrderMapper");
			}
			String serviceStartTimeStr = sdfToTime.format(serviceStartTime);
			soSchedule.setServiceTimeStart(serviceStartTimeStr);
		}
		...
		if (schedule.getScheduleType() != null) {
			if (PublicAPIConstant.DATETYPE_FIXED.equalsIgnoreCase((schedule.getScheduleType()))) {
				...
				if (PublicAPIConstant.DATETYPE_PREFERENCES.equalsIgnoreCase((schedule.getScheduleType()))) {
					soSchedule.setServiceDateTypeId(3);
				}else{
					soSchedule.setServiceDateTypeId(2);
				}
				
				if (null != schedule.getServiceDateTime2() && !schedule.getServiceDateTime2().equals("")) {
					soSchedule.setServiceDate2(DateUtils.defaultFormatStringToDate(schedule.getServiceDateTime2()));
					// setting the serviceEndTime
					Date serviceEndDate = null;
					try {
						serviceEndDate = CommonUtility.sdfToDate.parse(schedule.getServiceDateTime2());
					} catch (ParseException e) {
						logger.error("Parse Exception Occurred while " + "setting ServiceEndDate in OFServiceOrderMapper");
					}
					String serviceEndTimeStr = sdfToTime.format(serviceEndDate);
					soSchedule.setServiceTimeEnd(serviceEndTimeStr);
				}
	 ...
	 * @param serviceOrder
	 */
	private void updateServiceLocationContact(SOUpdateRequest request, ServiceOrder serviceOrder, OrderFulfillmentRequest orderRequest)
			throws ValidationException {

		if (StringUtils.isNotBlank(request.getOverView()) || null != request.getLocation() || null != request.getContact()) {

			if (!(OrderConstants.COMPLETED_STATUS == serviceOrder.getWfStateId() || OrderConstants.CLOSED_STATUS == serviceOrder
					.getWfStateId())) {
	...
	private void mapSoLocation(SOUpdateRequest request, ServiceOrder serviceOrder, OrderFulfillmentRequest orderRequest) {
		// Mapping service order location details
	...
	private void mapSoOverview(SOUpdateRequest request, ServiceOrder serviceOrder, OrderFulfillmentRequest orderRequest) {
		// Mapping service order overview
	...
	public BuyerResource getBuyerResource(int buyerId, Integer resourceId) {
		...
		try {
			...
			logger.error("Data Exception Occurred while getting " + " buyer resource details " + e);
		}
		return buyerResource;
	}
	...
	public boolean isResourceAbuyerAdmin(String userName) {
		...
		try {
			...
			logger.error("Data Exception Occurred while getting " + " buyer  details " + e);
		}
		return isAdmin;
	}
	 ...
	 * @param securityContext
	 * @throws ValidationException
	 *             This method calls the respective mapping methods for update
	 *             API call
	 */
	public StringBuilder mapToUpdate(ServiceOrder serviceOrder, SOUpdateRequest request, OrderFulfillmentRequest orderRequest,
		...
		if (serviceOrder != null) {
			if (null == request.getSpendLimitIncrease() && null == request.getSpendLimitDecrease() && null == request.getCustomReferences()
					&& StringUtils.isBlank(request.getOverView()) && null == request.getLocation() && null == request.getContact()
					&& StringUtils.isBlank(request.getSoSubstatus()) && null == request.getSchedule() && null == request.getScheduleServiceSlot()) {
				...
				if (null != request.getSpendLimitIncrease()) {
					...
					if (isSpendLimitEditable(serviceOrder)) {
						loggingCommentForSpendLimit = mapSpendLimit(serviceOrder, request.getSpendLimitIncrease(), buyerId,
								validationErrors, buyerResource, isAdmin);
				...
				if (null != request.getCustomReferences()) {
					...
					loggingCommentForCustRef = mapServiceOrderCustomRefForUpdate(serviceOrder, request.getCustomReferences(), buyerId,
							validationErrors);
				...
				if (StringUtils.isNotBlank(request.getOverView()) || null != request.getLocation() || null != request.getContact()) {
					if(isLocationEditable(serviceOrder)){
						...
						validationErrors
						.add(CommonUtility.getMessage(PublicAPIConstant.INVALID_UPDATE__RESULT_CODE));

				}

				if (!(validationErrors.isEmpty())
						&& (validationErrors.contains(PublicAPIConstant.INVALID_STATE_SPEND_LIMIT) || validationErrors
								.contains(PublicAPIConstant.INVALID_STATE_SPEND_LIMIT_RELAY))
	...
	/**
	 * @param serviceOrder
	 * @return returns true if service order is in a state to update customer
	 *         reference for update API call
	 * 
	 */
	/*private boolean isCutsomerReferenceEditable(ServiceOrder serviceOrder) {
		int statusId = serviceOrder.getWfStateId();
		return statusId == OrderConstants.ACCEPTED_STATUS || statusId == OrderConstants.PROBLEM_STATUS
				|| statusId == OrderConstants.ACTIVE_STATUS || statusId == OrderConstants.COMPLETED_STATUS
				|| statusId == OrderConstants.CLOSED_STATUS;
	} */

	/**
	 * @param serviceOrder
	 * @return returns true if service order is in a state to have spend limit
	 *         increase for update API call
	 * 
	 */
	private boolean isSpendLimitEditable(ServiceOrder serviceOrder) {
		int statusId = serviceOrder.getWfStateId();
		return statusId == OrderConstants.ACCEPTED_STATUS || statusId == OrderConstants.PROBLEM_STATUS
	...
	/**
	 * @param serviceOrder
	 * @return returns true if service order is in a state to have update location
	 *          for update API call
	 * 
	 */
	private boolean isLocationEditable(ServiceOrder serviceOrder) {
		int statusId = serviceOrder.getWfStateId();
	...
	public StringBuilder mapSpendLimit(ServiceOrder serviceOrder, SpendLimitIncrease spendLimitIncrease, int buyerId,
			List<String> validationErrors, BuyerResource buyerResource, boolean isAdmin) {
		...
		// Admin can do what ever he/she wants - Do they have some max limit? -
		// Yes
	...
	public StringBuilder mapSpendLimitRelay(ServiceOrder serviceOrder, SpendLimitDecrease spendLimitIncrease, int buyerId,
			List<String> validationErrors, BuyerResource buyerResource, boolean isAdmin) {
		StringBuilder loggingComment = new StringBuilder("");
		String reason;
		String notes;
		...
		/*
		 * if((!isAdmin) || (isAdmin && buyerResource.getMaxSpendLimitPerSO() >
		 * 0.00) ){ BigDecimal maxLimitAllowed = new
		 * BigDecimal(buyerResource.getMaxSpendLimitPerSO()); BigDecimal
		 * difference = new BigDecimal(0.00); // Find out the difference between
		 * current price and updated price
		 * if(finalMaxPrice.compareTo(originalMaxPrice)>0){ difference =
		 * finalMaxPrice.subtract(originalMaxPrice); }
		 * if(difference.compareTo(maxLimitAllowed)>0){ // Error!
		 * validationErrors
		 * .add(CommonUtility.getMessage(PublicAPIConstant.SPEND_LIMIT_EXCEEDED
		 * )); return loggingComment; } }
		 */
		 ...
		 * if(finalMaxPrice.compareTo(originalMaxPrice)>0){
		 * serviceOrder.setSpendLimitLabor(spendLimitLabor);
		 * serviceOrder.setSpendLimitParts(spendLimitParts); String
		 * priceAfterFormatting = "";
		 * if(originalSpendLimitLabor.compareTo(spendLimitLabor)!=0){
		 * priceAfterFormatting =
		 * priceFormatter.format(spendLimitLabor.doubleValue());
		 * loggingComment.append
		 * ("Labor Spend Limit Changed to  ====>  $"+priceAfterFormatting
		 * +"<br>"); }
		 * if(originalSpendLimitParts.compareTo(spendLimitParts)!=0){
		 * priceAfterFormatting =
		 * priceFormatter.format(spendLimitParts.doubleValue());
		 * loggingComment.append
		 * ("Materials Spend Limit Changed to  ====>  $"+priceAfterFormatting
		 * +"<br>"); } priceAfterFormatting =
		 * priceFormatter.format(finalMaxPrice.doubleValue());
		 * loggingComment.append
		 * ("Total SpendLimit  ====>  $"+priceAfterFormatting+"<br>"); }else
		 */if (finalMaxPrice.compareTo(originalMaxPrice) < 0) {
	 ...
	 * @param spendLimitIncrease
	 * @param serviceOrder
	 * @return method to map price to SO Price
	 */

	public SOPrice mapPricing(SpendLimitIncrease spendLimitIncrease, ServiceOrder serviceOrder) {
		...
		returnVal.setDiscountPercentLaborSL(calculatePercentage(spendLimitIncrease.getLaborDiscount(), spendLimitIncrease.getLaborSpendLimit()));
		returnVal.setDiscountPercentPartsSL(calculatePercentage(spendLimitIncrease.getPartsDiscount(), spendLimitIncrease.getPartsSpendLimit()));
		/*
		returnVal.setTaxPercentLaborSL(StringUtils.isEmpty(spendLimitIncrease.getLaborTax()) ? PricingUtil.ZERO : new BigDecimal(
				spendLimitIncrease.getLaborTax()));
		returnVal.setTaxPercentPartsSL(StringUtils.isEmpty(spendLimitIncrease.getPartsTax()) ? PricingUtil.ZERO : new BigDecimal(
				spendLimitIncrease.getPartsTax())); 
		*/
	 ...
	 * @return
	 */
	private BigDecimal calculatePercentage(String absoluteValue, String spendLimit) {
		
		BigDecimal percent = null;
		
		if (!StringUtils.isEmpty(absoluteValue) && !StringUtils.isEmpty(spendLimit)) {
			BigDecimal absDisc = new BigDecimal(absoluteValue);
			BigDecimal spendLim = new BigDecimal(spendLimit);
			if (absDisc.compareTo(BigDecimal.ZERO) > 0 && spendLim.compareTo(BigDecimal.ZERO) > 0) {
				percent = absDisc.divide(spendLim, 4, RoundingMode.HALF_EVEN);
			} else {
				percent = PricingUtil.ZERO;
			}
		} else {
			percent = PricingUtil.ZERO;
		}
	...
	/**
	 * 
	 * @param spendLimitDecrease
	 * @param serviceOrder
	 * @return
	 */
	public SOPrice mapPricing(SpendLimitDecrease spendLimitDecrease, ServiceOrder serviceOrder) {
		...
		// set tax and discount
		returnVal.setDiscountPercentLaborSL(calculatePercentage(spendLimitDecrease.getLaborDiscount(), spendLimitDecrease.getLaborSpendLimit()));
		returnVal.setDiscountPercentPartsSL(calculatePercentage(spendLimitDecrease.getPartsDiscount(), spendLimitDecrease.getPartsSpendLimit()));
		
		/*
		returnVal.setTaxPercentLaborSL(StringUtils.isEmpty(spendLimitDecrease.getLaborTax()) ? PricingUtil.ZERO : new BigDecimal(
				spendLimitDecrease.getLaborTax()));
		returnVal.setTaxPercentPartsSL(StringUtils.isEmpty(spendLimitDecrease.getPartsTax()) ? PricingUtil.ZERO : new BigDecimal(
				spendLimitDecrease.getPartsTax()));
		*/
	 ...
	 */
	/* private StringBuilder mapServiceOrderCustomRef(ServiceOrder serviceOrder, CustomReferences customReferences, Integer ownerId,
		...
		if (existingCustomRefs != null) {
			for (SOCustomReference customRef : existingCustomRefs) {
				if (StringUtils.isNotBlank(customRef.getBuyerRefTypeName()) && StringUtils.isNotBlank(customRef.getBuyerRefValue())) {
			...
			for (CustomReference customRef : customReferences.getCustomRefList()) {
				...
				if (null != buyerRefVO && null != buyerRefVO.getRefTypeId()) {
					...
					if (buyerRefVO.getEditable() == 1) {
						...
						// Get the value if it is already present in the service
						// order.
						...
						validationErrors.add(customRef.getName() + " "
								+ CommonUtility.getMessage(PublicAPIConstant.NOTEDITABLE_CUST_REF_VALUE));
					}
				} else {
	...
	*/
	 ...
	 *            maps the customer reference for update API call
	 */
	private StringBuilder mapServiceOrderCustomRefForUpdate(ServiceOrder serviceOrder, CustomReferences customReferences, Integer ownerId,
			List<String> validationErrors) {
		StringBuilder loggingComment = new StringBuilder("");
		List<SOCustomReference> soCustomRefsVoList = new ArrayList<SOCustomReference>();
		List<SOCustomReference> existingCustomRefs = serviceOrder.getCustomReferences();
		...
		if (null != customReferences && null != customReferences.getCustomRefList()) {
				...
				if (null != buyerRefVO && null != buyerRefVO.getRefTypeId()) {
					...
					if (buyerRefVO.getEditable() == 1) {
						...
						if (null != existingCustomRefValue) {
							...
							loggingComment.append("The reference field " + customRefName + " has been updated to " + customRefValue
									+ "<br>");
							updatedCustRefs.add(custRefVO);
						}
					} else if (buyerRefVO.getEditable() == 0) {
			...
			int statusId = serviceOrder.getWfStateId();
			if (validationErrors.isEmpty()
					&& (statusId == OrderConstants.CLOSED_STATUS || statusId == OrderConstants.DELETED_STATUS
							|| statusId == OrderConstants.VOIDED_STATUS || statusId == OrderConstants.CANCELLED_STATUS)) {
	...
	/**
	 * SL-20400 : Creating response to show duplicate service order for InHome
	 * Buyer (3000)
	 * 
	 * @param servOrder
	 * @return
	...
	private Integer validateSoSubstatus(ServiceOrder serviceOrder, String soSubStatus) {
		try {
			...
			logger.error("Data Exception Occurred while validating So Substatus" + e.getMessage(), e);
		}
		return null;
	}
	...
	private void validateDates(ServiceOrder serviceOrder, List<String> validationErrors) {
		...
		Timestamp currentDateTime = new Timestamp(now.getTime());

		com.servicelive.orderfulfillment.domain.SOSchedule soSchedule = serviceOrder.getSchedule();

		Timestamp timeStampStartDate = new Timestamp(soSchedule.getServiceDate1().getTime());

		Timestamp newStartTimeCombined = new Timestamp(TimeUtils.combineDateAndTime(timeStampStartDate, soSchedule.getServiceTimeStart(),
				serviceOrder.getServiceLocationTimeZone()).getTime());
		if (null != soSchedule.getServiceDate2()) {
			Timestamp timeStampEndDate = new Timestamp(soSchedule.getServiceDate2().getTime());

			Timestamp newEndTimeCombined = new Timestamp(TimeUtils.combineDateAndTime(timeStampEndDate, soSchedule.getServiceTimeEnd(),
					serviceOrder.getServiceLocationTimeZone()).getTime());
