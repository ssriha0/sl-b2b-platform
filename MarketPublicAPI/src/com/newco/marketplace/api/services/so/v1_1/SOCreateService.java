...
import com.newco.marketplace.api.utils.mappers.so.v1_1.SOCreateMapper;
...
/**
 * This is the service class for executing the Create Service Order request.
 * 
 * @author Infosys
 * @version 1.0
 */
...
public class SOCreateService extends BaseService {
	...
	public IAPIResponse execute(APIRequestVO apiVO) {
		...
		if (securityContext == null) {
			...
			HashMap<String, Object> soMap = null;
			
				
//			} else {
				
				try {
					soMap = createMapper.mapServiceOrder(createRequest, securityContext);
				} catch (DataException e) {
					logger.error("SOCreateService.execute(): DataException occured: ", e);
					return createGenericErrorResponse();
				}
				...
				if(null != soMap.get(PublicAPIConstant.ERROR_KEY)){
					...
					if(errorList.hasNext()){
						...
						return createErrorResponse(errorMessage, ResultsCode.INVALID_OR_MISSING_REQUIRED_PARAMS.getCode());
					}
				}
				serviceOrder = (ServiceOrder) soMap.get(PublicAPIConstant.SERVICE_ORDER);
				List titleList = (List) soMap.get(PublicAPIConstant.FILE_TITLE);
				logger.info("Create SO Mapping Completed. Now calling ServiceOrderBO.processCreateDraftSO() for actual SO Creation");

				ProcessResponse processResponse = serviceOrderBO.processCreateDraftSO(serviceOrder, securityContext);
				logger.info("Association of Buyer Documents with SOId--->starts");
				invalidDocumentList = associateDocumentsWithSO((String) processResponse.getObj(), titleList, securityContext.getCompanyId(), createRequest.getServiceOrder().getAttachments());
...
//			}
	...
	/**
	 * Associate the attachements with the service order.
	 * @param soId
	 * @param titleList
	 * @param ownerId
	 * @param attachments
	 * @return
	 */
    private List<String> associateDocumentsWithSO(String soId, List titleList, Integer ownerId, Attachments attachments){
		List<String> invalidDocumentList = new ArrayList<String>();
		if (!soId.equalsIgnoreCase(PublicAPIConstant.INVALID_SOID)
				&& (null != titleList) && titleList.size() > 0) {
			associateSOWithDocumentsForTitle(soId, ownerId, titleList,invalidDocumentList);
		} else if (null != attachments && null != attachments.getFilenameList()
				&& (!soId.equalsIgnoreCase(PublicAPIConstant.INVALID_SOID))) {
			List<String> filenameList = attachments.getFilenameList();
			if (!filenameList.isEmpty()) {
				invokeDocumentList(soId, ownerId, filenameList,invalidDocumentList);
