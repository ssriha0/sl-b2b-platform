	...
	public void process() throws SLBusinessServiceException {
		...
		try {
			...
			for (TransactionRecordVO transactionRecordVO : al) {
				...
				String cardInitialSixDigits = transactionRecordVO.getMaskedAccountNo().substring(0, 6);
				if (StringUtils.equals(cardInitialSixDigits,CommonConstants.SYW_CARD_BIN)){
					transactionRecordVO.setCardTypeCode(CommonConstants.SYW_PAYMENT_TYPE_CODE);
				}
			...
			if ((consolidatedRollUpDepositAmt.doubleValue() != creditCardDepositsSum) || (consolidatedRollUpRefundAmt.doubleValue() != MoneyUtil.getRoundedMoney(creditCardRefundsSum))) {
				...
				String encryptScriptFilePath = this.applicationProperties.getPropertyFromDB("encrypt_script_file_path");
				Runtime.getRuntime().exec(encryptScriptFilePath + " " + fullFileName);
			}
