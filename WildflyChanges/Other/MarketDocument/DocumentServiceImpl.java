...
//         	  documentVO = retrieveDocumentFromDBOrFS(documentVO);   
	...
	public String buildDocumentPath(DocumentVO docVO)throws DocumentRetrievalException{
		...
		{
			String legalNotice=docVO.getDocSource();
			if(null!=legalNotice && legalNotice.equalsIgnoreCase(MPConstants.PROVIDER_LEGAL_NOTICE_INDICATOR))
			{
				
				String legalNoticetSaveLocation="";
				try {
					legalNoticetSaveLocation = documentDao.getAppPropertiesValue(MPConstants.LEGAL_NOTICE_SAVE_LOCATION);
					logger.info("Legal notice save location"+legalNoticetSaveLocation);
					docVO.setDocPath(legalNoticetSaveLocation);
				} catch (DataServiceException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
