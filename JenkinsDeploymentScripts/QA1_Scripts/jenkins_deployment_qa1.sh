...
source $1/JenkinsDeployementScripts/QA1_Scripts/jenkins_deployment_qa1.config
source $1/JenkinsDeployementScripts/QA1_Scripts/revert_failed_build_dbscripts_qa1.sh
...
undeploy_war() {
	...
	deploy_war "" $all_batch_hosts $all_deployables_batch $all_script_spn_batch $all_sl_spn_batch_war_name $jenkins_path_spn $log_file_path_batch
	...
	while [ $i -le $count ]; do
		...
		if [ $? -eq 0 ]; then
			...
			if [ $? -eq 0 ]; then
				...
				cho "ERROR LOG: Failed Deployment of StaticWeb.war"
			fi;	
		else
