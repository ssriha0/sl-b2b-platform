...
source $1/JenkinsDeployementScripts/API_Test1_Scripts/revert_failed_build_dbscripts_api.sh
...
undeploy_war() {
	...
	deploy_war "" $all_callback_hosts $all_deployables_callback $all_script_callback $all_callback_war_name $jenkins_path_callback $log_file_path_api
	...
	while [ $i -le $count ]; do
		...
		if [ $? -eq 0 ]; then
			...
			if [ $? -eq 0 ]; then
				...
				eecho "SUCCESS LOG: Deployment of StaticWeb.war done successfully"
			else
