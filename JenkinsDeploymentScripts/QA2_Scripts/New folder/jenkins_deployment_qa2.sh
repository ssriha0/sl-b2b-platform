...
deploy_war(){
	...
	echo "Deploying on $2"
	echo "Pasting at $deployables_jvm"
	echo "App name for deploy $deploy_script_jvm"	
	echo "Copying $5"
	...
	ssh -i $path_to_private_key $user@$jvm_hosts "/appl/sm/jboss/manageApp82 deploy $deploy_script_jvm"
		...
		if [ $? -eq 0 ]; then
			...
			if [ $temp1 -eq 1 ]; then
					echo "Deployment of $jvm_war is SUCCESSFUL at location $deployables_jvm"					
				...
				echo "Deployment of $jvm_war FAILED with below ERRORS::::::" 	
				print_error="grep -B5 -A20 'ERROR' $jvm_log"
				ssh -i $path_to_private_key $user@$jvm_hosts "$print_error"
				
				echo "Deployment of $jvm_war is Failed. Reverting the current build."					
				undeploy_war 
				revert_build_db_scripts		
				echo "Build is reverted successfully"
				exit 1;
			fi;	
		fi;		
		i=$(($i+1))
...
undeploy_war() {
			...
			while [ $revert_jvm_count -gt 0 ]; do				
				...
				echo "**Deploying Old last $old_war :: $old_war"
