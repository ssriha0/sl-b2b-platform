...
echo "Starting deployement of WAR file"
	...
	while [ $i -le $count ]; do
		...
		if [ $? -eq 0 ]; then			
			...
			if [ $temp1 -eq 1 ]; then
				echo "Deployment of $public_War is SUCCESSFUL at location $all_deployables_api"
			else 
		...
		if [ $? -eq 0 ]; then
			...
			if [ $temp1 -eq 1 ]; then
				echo "Deployment of $callback_War is SUCCESSFUL at location $all_deployables_api"
			else 
	...
	while [ $i -le $count ]; do
		...
		script_spn_batch=`(cut -d "," -f$i <<< ${all_script_spn_batch})`
		
		marketBatch_War=`(cut -d "," -f$i <<< ${all_sl_batch_war_name})`
		spn_batch_War=`(cut -d "," -f$i <<< ${all_sl_spn_batch_war_name})`
		...
		#scp -i $path_to_private_key $jenkins_path_spn_batch $user@$batch_hosts:$deployables_batch
		#
		#ssh -i $path_to_private_key $user@$batch_hosts "/appl/sm/jboss/manageApp82 deploy $script_spn_batch"
		#if [ $? -eq 0 ]; then
		#	temp="grep -F -c 'Deployed \"$spn_batch_War\"' $log_file_path_batch"  
		#	temp1=`ssh -i $path_to_private_key $user@$batch_hosts "$temp"`
		#		
		#	if [ $temp1 -eq 1 ]; then
		#		echo "Deployment of $spn_batch_War is SUCCESSFUL at location $deployables_batch"
		#	else 
		#		echo "Deployment of $spn_batch_War is Failed "
		#		exit 1
		#	fi;		
		#	echo "Deployment of $spn_batch_War is SUCCESSFUL at location $deployables_batch"
		#else
