...
public class JobExceptionHandler implements Synchronization, Command<Object> {
  ...
  public Object execute(Environment environment) throws Exception {
    ...
    if (job != null) {
      ...
      job.release();
      String causeMessage = sw.toString();
      log.info(causeMessage);
// BEGIN PATCHED CODE
...
// keywords to retry the transaction.
      if(causeMessage.matches(".*try restarting transaction$")){
    	  log.info("Deadlock detected retry transaction. Not decrementing the retries");
    	  decrementedRetries++;
      }
// END PATCHED CODE	  
      job.setRetries(decrementedRetries);
	  job.setException(causeMessage);
