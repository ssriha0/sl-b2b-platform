...
//    private static Logger logger = Logger.getLogger(RestClient.class);
    ...
    public JerseyRestClient(String oAuthEnabled,String consumerKey,String consumerSecret,String baseUrl){
    	...
    	client.setConnectTimeout(OrderConstants.API_CONNECTION_TIMEOUT);
    ...
    public <T> T post(Class<T> clazz, Object payload){
        ...
        MultivaluedMap<String,String> _params = parameters;
        clearState();
        T returnVal = _resource.queryParams(_params).accept(MediaType.APPLICATION_XML).type(MediaType.TEXT_XML).post(clazz, payload);
        return returnVal;
    }
    ...
    public <T> T put(Class<T> clazz, Object payload) {
        WebResource _resource = resource;
        clearState();
        T returnVal = _resource.accept(MediaType.APPLICATION_XML).type(MediaType.TEXT_XML).put(clazz, payload);
        return returnVal;
    }
    ...
    public <T> T getJsonResponse(Class<T> aClass) {
        T returnVal = null;
        MultivaluedMap<String,String> _params = parameters;
        WebResource _resource = resource;
        clearState();
        if(!_params.isEmpty()){
        	returnVal = _resource.queryParams(_params)
                .accept(MediaType.APPLICATION_JSON).header("Content-type", "application/json")
                .get(aClass);
        }else{
