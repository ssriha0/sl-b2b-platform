...
public class MPUtils {
	...
	public static String getErrorMessage(String errorCode) {
				...
				try {
					if (resouceBundle == null) {
						resouceBundle = ResourceBundle.getBundle("com.sears.rfs.utils.ErrorMessages");
					}
	...
	/**
		...
		{
			if((strErrorCode!= null)&& (strErrorCode.equals(MPConstants.SYSTEMERROR)))
				return true;
			else
				return false; 
		}
		...
		{
			if((strErrorCode!= null)&& (strErrorCode.equals(MPConstants.BUSINESSERROR)))
				return true;
			else
				return false; 
		}
		 ...
		 */
	public static boolean isNotValidCode(String ccNum) {
	 ...
	 */
		...
		public static boolean checkNumericLength(String strValue,int intIntegerLength,int intDecimalLength)
		{
			...
			{ 					
				...
				strDecimalPart = strValue.substring(intIndex+1,strValue.length());															
				if(strDecimalPart.length() == 0)
					return false;
				if((strIntegerPart.length() > intIntegerLength) || (strDecimalPart.length() > intDecimalLength))
					return false;
				else
					return true;
			}
	...
	public static boolean checkFutureDate(String strDate)
	{
		...
		SimpleDateFormat sdf = new SimpleDateFormat(DATE_FORMAT);
		Calendar c1 = Calendar.getInstance();
		c1.set(Integer.parseInt(strHldTokens[2]),Integer.parseInt(strHldTokens[0])-1,Integer.parseInt(strHldTokens[1])); 	  
		Date today = new Date();
		String datenewformat = sdf.format(today);
		try {
			d1 = sdf.parse(datenewformat);
	...
	{
		String strHldTokens[] = getDateTokens(strDate);
		if (strHldTokens[0].length()!=2 || strHldTokens[1].length()!=2 || strHldTokens[2].length()!=4){
			return false;
		}
		return true;
	}	
	...
	public static boolean invalidDate(String strDate)
	{
		String strHldTokens[] = getDateTokens(strDate);
		try{
			if (Integer.parseInt(strHldTokens[0])>12 || Integer.parseInt(strHldTokens[1])>31){
				return false;
			}
		}
		catch(Exception ex){
	...
	/**
		...
		public static int getDateDifference(String strFromDate, String strToDate)
		{
			...
			Date fromDate = new GregorianCalendar(Integer.parseInt(strHldTokens[2]),Integer.parseInt(strHldTokens[0]), Integer.parseInt(strHldTokens[1])).getTime();
    		/** Today's date */
			//yyy/mm/dd
			Date toDate = new GregorianCalendar(Integer.parseInt(strHldTokens[2]),Integer.parseInt(strHldTokens[0]), Integer.parseInt(strHldTokens[1])).getTime();
	 ...
	 */
		public static String assignFromDate(String strToDate)
		{
			...
			Calendar c1 = Calendar.getInstance();
			c1.set(Integer.parseInt(strHldTokens[2]),Integer.parseInt(strHldTokens[0]),Integer.parseInt(strHldTokens[1])); 	  
		...
		public static String prependZeroes(String strParam,int intNumberOfZeroes)
		{
			...
			{
				intSonoLength = strParam.length();
				//if(intSonoLength < 7)
				//{
					//intNumberOfZeroes = (4 - intSonoLength);
					for(int i=0; i< intNumberOfZeroes; i++)
						strZero = strZero + "0";
		...
		try {
			 ...
			 if (!date.equals(formatter.format(dateSimple))) return false;
 			  return true;
   		  	 }
		  	 catch(ParseException e) {
			 	return false;
