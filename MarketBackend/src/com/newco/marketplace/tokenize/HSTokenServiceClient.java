...
public class HSTokenServiceClient {
	private static final Logger LOGGER = Logger.getLogger(HSTokenServiceClient.class);	
		...
		try{
			...
			if(null != homeServiceResponseVO){
			   LOGGER.info("homeServiceResponseVO ResponseXML:"+ homeServiceResponseVO.getResponseXml());
				...
				throw new BusinessServiceException("Exception in invoking Tokenize Home Service Web service from the Client");
			}
		}catch (Exception e) {
			LOGGER.error("Exception in invoking Home Service Tokenize Web service from the Client"+ e);
