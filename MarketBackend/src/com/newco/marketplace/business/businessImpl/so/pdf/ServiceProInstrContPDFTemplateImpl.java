...
public class ServiceProInstrContPDFTemplateImpl implements PDFTemplate {
	
	private Logger logger = Logger.getLogger(ServiceProInstrContPDFTemplateImpl.class);
	...
	private void addSpecialInstProvList(PdfPTable serviceProInstrContTable ){
		...
		if(so.getProviderInstructions() != null){
			...
			String terms = so.getProviderInstructions();
			try {
				//replace html character entities
				terms = SOPDFUtils.getHtmlContent(terms);
				//Convert html to rich text
				termsAndConditions = SOPDFUtils.getRichText(terms, SOPDFConstants.FONT_LABEL, null, SOPDFConstants.SPACE);
			} 
			catch (Exception e) {
			...
			if(null != termsAndConditions && !termsAndConditions.isEmpty()){
				specialInstProvList.add(termsAndConditions);
			}
			else{
				terms = so.getProviderInstructions();
				//Remove html tags
				terms = ServiceLiveStringUtils.removeHTML(terms);
				specialInstProvList.add(new ListItem(terms, SOPDFConstants.FONT_LABEL));
			}
		}	
