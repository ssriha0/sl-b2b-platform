...
<script type="text/javascript">
	jQuery(document).ready(function($) {
		initShowHideCategories();	
		setupMultiSelect();		
		setupSubCategoryNetwork();		
	});
</script>
			...
			<div class="half">
				...
				<div class="select-options selectOptionsSub">	
						<c:forEach items="${subServices.value}" var="subService">
							...
							<div style="clear: left;"><input type="checkbox" style="clear: none;" name="approvalItems.selectedSubServices1" value="${subService.id}" ${checked} /> ${subService.description}</div>
						</c:forEach>
				</div>
			</div>
		</c:if>
