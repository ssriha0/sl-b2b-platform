...
public class ServiceLiveStringUtils extends StringUtils{
	...
	private static String suffixString(String inputString, int finalLen) {
		...
		try {
			...
			if (appendLen > 0) {
				...
				resultString = inputString + concatString.toString();
			}
			else {
				resultString = inputString;
			}
	 ...
	 * @return
	 */
	public static String concatenateCityStateZip(String city, String state,
	...
	/**
	 * function will mask the alpha numeric string with the given character
	 * */
	public static String maskStringAlphaNum(String input, int clearTextLen, String maskChar) {
		String finalString = null;
		String clearText = null;
		if(input != null && (!("").equals(input))){
			int inputLen = input.length();
			...
			clearText = input.substring(inputLen - clearTextLen);
			finalString = input.substring(0, inputLen - clearTextLen).replaceAll("[^A-Za-z0-9]", maskChar);
			finalString = finalString .substring(0, inputLen - clearTextLen).replaceAll("[A-Za-z0-9]", maskChar) + clearText;			
		}
	...
	/**
     * <p>Replaces all occurrences of new line characters within a String.</p>
	...
	/**
	 * Method to remove the html tags from the string
	 * @param htmlString
	 * @return
	 */
	  public static String removeHTML(String htmlString){
	         // Remove HTML tag from java String    
	    	 //String noHTMLString = htmlString.replaceAll("</li>","\n\n");
		  String noHTMLString ="";
		  if(StringUtils.isNotBlank(htmlString)){
		     noHTMLString = htmlString.replaceAll("/>\\s+</g", "><");
