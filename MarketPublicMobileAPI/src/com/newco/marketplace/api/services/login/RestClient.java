...
//    private static Logger logger = Logger.getLogger(RestClient.class);
    ...
    public <T> T get(Class<T> aClass) {
        ...
        if(!_params.isEmpty()){
            ...
            returnVal = _resource.accept("application/xml")
                .get(aClass);
        }
        return returnVal;
    }
    ...
    public <T> T post(Class<T> clazz, Object payload){
        WebResource _resource = resource;
        clearState();
        T returnVal = _resource.accept("application/xml").type("text/xml").post(clazz, payload);
        return returnVal;
    }
    ...
    public <T> T put(Class<T> clazz, Object payload) {
        WebResource _resource = resource;
        clearState();
        T returnVal = _resource.accept("application/xml").type("text/xml").put(clazz, payload);
        return returnVal;
    }
    ...
    public <T> T delete(Class<T> clazz) {
        ...
        if(!_params.isEmpty()){
            ...
            returnVal = _resource.accept("application/xml").delete(clazz);
        }
        return returnVal;
    }
