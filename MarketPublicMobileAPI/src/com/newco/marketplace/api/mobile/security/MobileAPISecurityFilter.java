...
public class MobileAPISecurityFilter implements Filter {
	...
	//SL-20098: Changing the messages to prevent verbose messages in the response
	private final String errorString = "";//"Not enough permissions to access URL";
	private final String oAuthFailed = "";//"Unauthorized access. Please validate your consumer key and consumer secret";
	...
	public void doFilter(ServletRequest servletRequest,
		...
		try {
			...
			String requestToken = simpleOAuthValidation(requestMessage,
					consumerKey, secret);
			...
			if (StringUtils.isNotBlank(inputPath)
					&& (!inputPath.contains("/provider/authenticate") && !inputPath.contains("/provider/validateMobileVersion")
							&& !inputPath.contains("/provider/updateMobileAppVersion"))) {
				validationMessage = new StringBuilder();
				if(validateResourceAndToken(response, resourceId, requestMessage,
						inputPath, requestToken, validationMessage)){
			...
			if (userId != -1) {
				boolean flag = mobileApiSecurity.isUserExisting(userId,
						consumerKey, roleId);
				if (flag == false) {
					logger.info("Consumer(" + consumerKey
							+ ") is not linked to user(" + userId + ")");
			...
			boolean flag = isOperationAllowed(inputPath, consumerKey,
					request.getMethod());
			if (flag == false) {
				logger.info("API operation(" + inputPath
						+ ") not allowed for consumer:" + consumerKey);
				response.sendError(HttpServletResponse.SC_FORBIDDEN,
						errorString);
				return;
			}
	...
	private boolean validateResourceAndToken(HttpServletResponse response,
			String resourceId, OAuthMessage requestMessage, String inputPath,
			String requestToken, StringBuilder validationMessage) throws BusinessServiceException,
		...
		if (null != requestMessage && null != requestToken) {
			...
			if (StringUtils.isNotBlank(resourceId)) {
				...
				boolean validResource = mobileApiSecurity
						.isResourceValid(Integer.parseInt(resourceId));
				...
				if (!validResource) {
					...
					boolean validToken = mobileApiSecurity.isMobileTokenValid(
							requestToken, resourceId,
							serverDate);
	...
	private String simpleOAuthValidation(OAuthMessage requestMessage,
			String consumerKey, String secret) throws IOException,
	...
	private boolean isOperationAllowed(String url, String consumerKey,
			String httpMethod) {
	...
	private String maskUrl(String url) {
		...
		if (url.indexOf("serviceorder") != -1) {
			...
			if (m.find() == true)
				url = url.replaceAll("serviceorder/[0-9-_]+/",
						"serviceorder/{fid}/");
			else
		...
		if (url.indexOf("serviceorders") != -1) {
			...
			if (m.find() == true)
				url = url.replaceAll("serviceorders/[0-9-_]+/",
						"serviceorders/{fid}/");
			else
		...
		if (url.indexOf("downloadDocument") != -1) {
			url = url.replaceAll("downloadDocument/[0-9]+",
					"downloadDocument/{docid}");
		}
