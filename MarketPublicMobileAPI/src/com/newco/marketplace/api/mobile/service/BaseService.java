...
package com.newco.marketplace.api.mobile.service;
...
import com.newco.marketplace.api.mobile.security.APIFieldMasker;
import com.newco.marketplace.api.mobile.security.SecurityProcess;
...
import com.newco.marketplace.mobile.api.utils.validators.ReqResValidator;
...
public abstract class BaseService {
	...
	//SLT-1164
	public String validateCC(String resolutionComments) {

		String responseXML = null;
		if (resolutionComments != null && CreditCardValidatonUtil.validateCCNumbers(resolutionComments)) {
			logger.info("Entering cc validations");
			IAPIResponse response;
			try {
				response = (IAPIResponse) this.responseClass.newInstance();
			} catch (InstantiationException e) {
			...
			ErrorResult errorResult = new ErrorResult();
			errorResult.setMessage(PublicMobileAPIConstant.CC_VALIDATION_MSG);
			errorResult.setCode("CCValidation");

			errorList.add(errorResult);
			results.setResult(resultList);
			results.setError(errorList);
			response.setResults(results);
			responseXML = getCreateResponseXML(response);
			logger.info("Leaving cc validations");
			return responseXML;
		}
	...
	public FileData getBytesFromFile(MultipartBody request) {
		...
		try {
			...
			List<Attachment> attachments = request.getAllAttachments();
			/*
			 * Every multipart entity which is set in the request is mapped as
			 * one ATTACHMENT in cfx MultiPart Iterating through these
			 * attachments, we will get all the fields passed as request.
			 */
			for (Attachment attachment : attachments) {

				// Develop a DataHandler for the data being passed as an
				// attachment.
				DataHandler attachmentHandler = attachment.getDataHandler();
				MultivaluedMap<String, String> formDetailsMap = attachment
						.getHeaders();

				// We use Content-Disposition header, when dealing with the form
				// submission of files.
				String[] formContentDisposition = formDetailsMap.getFirst(
				...
				for (String fields : formContentDisposition) {
					...
					if (fields.trim().startsWith("filename")) {
						...
						try {
							inputStream = attachmentHandler.getInputStream();
							bytes = IOUtils.toByteArray(inputStream);

						} catch (IOException e) {
							logger.info("Exception occureed while reading the file due to "+e);
						}
			...
			fileData.setByteString(new String(bytes));
			return fileData;
		} catch (Exception e) {
			logger.info("Error in converting file in request to bytes" + e);
			return null;
		}
