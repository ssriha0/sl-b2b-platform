...
public class UpdateSchedulePreCallTest {
	...
	public void testMapUpdateTimeWindowRequestForNoCustomer(){
		...
		Assert.assertEquals(updateScheduleVO.getSource(), "PRE_CALL");
		...
		Assert.assertEquals(updateScheduleVO.getScheduleStatusId(),MPConstants.PRE_CALL_ATTEMPTED);
	...
	public void testMapperForCustomerAvblConfirmAppt(){
		...
		Assert.assertEquals(updateScheduleVO.getScheduleStatusId(),MPConstants.PRE_CALL_COMPLETED);
		Assert.assertEquals(updateScheduleVO.getReasonId(), MPConstants.PRE_CALL_COMPLETED_REASON);
	...
	public void testInvalidSOStatus(){
		...
		try {
			...
			when(newMobileGenericBO.validateServiceOrderStatus(updateScheduleVO.getSoId(),MPConstants.PRE_CALL_ACTION)).thenReturn(false);
			
			response = mobileValidator.validateUpdateScheduleRequest(updateScheduleVO);
		
			Assert.assertNotNull(response.getResults());
			Assert.assertNotNull(response.getResults().getError());
			Assert.assertEquals(response.getResults().getError().get(0).getCode(),"3113");
			Assert.assertEquals(response.getResults().getError().get(0).getMessage(),"Service Order should be in Accepted status for Pre-Call.");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void testInvalidSOScheduleStatus(){
		...
		try {
			...
			newMobileGenericBO = mock(NewMobileGenericBOImpl.class);
			mobileValidator.setNewMobileGenericBO(newMobileGenericBO);
			when(newMobileGenericBO.validateServiceOrderStatus(updateScheduleVO.getSoId(),MPConstants.PRE_CALL_ACTION)).thenReturn(true);
			when(newMobileGenericBO.validateScheduleStatus(updateScheduleVO.getSoId(),MPConstants.PRE_CALL_ACTION)).thenReturn(false);
	...
	public void testAppointmentTodayAndUnassign(){
		...
		try {
			...
			mobileValidator.setNewMobileGenericBO(newMobileGenericBO);
			when(newMobileGenericBO.validateServiceOrderStatus(updateScheduleVO.getSoId(),MPConstants.PRE_CALL_ACTION)).thenReturn(true);
			when(newMobileGenericBO.validateScheduleStatus(updateScheduleVO.getSoId(),MPConstants.PRE_CALL_ACTION)).thenReturn(true);
