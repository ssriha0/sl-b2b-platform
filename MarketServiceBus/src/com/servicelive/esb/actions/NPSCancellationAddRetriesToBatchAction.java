...
public class NPSCancellationAddRetriesToBatchAction extends AbstractIntegrationSpringAction {
	private static final Logger logger = Logger.getLogger(NPSCancellationAddRetriesToBatchAction.class);
	private Double retryHours = 4D;
	private Long maxDbTransactions = 1000L;
	
	public NPSCancellationAddRetriesToBatchAction() { super(); }
	
	public NPSCancellationAddRetriesToBatchAction(ConfigTree config) throws ConfigurationException { 
		super(config);		
	...
	public Message addRetriesToBatch(Message message) {
		try {
			String resendErrorMessage = this.getIntegrationServiceCoordinator()
					.performAddNpsCancellationRetriesToBatch(retryHours,
							maxDbTransactions);			
