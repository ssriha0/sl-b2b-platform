...
public class NPSCancelAuditNotificationAction extends AbstractIntegrationSpringAction {

	public NPSCancelAuditNotificationAction() { super(); }
	public NPSCancelAuditNotificationAction(ConfigTree config) throws ConfigurationException { super(config); }
	
	@Override
	protected Long getIntegrationId(String fileName) {
		return IntegrationName.NPS_CANCEL_AUDIT.getId();
	}

	@Override
	protected String getIntegrationName(String fileName) {
		return IntegrationName.NPS_CANCEL_AUDIT.name();
	}
	...
	private void notifyAuditOwners(List<OmsBuyerNotificationResponse> npsAuditOrders/*List<NPSAuditOrder> npsAuditOrders*/) {
		...
		for (Integer ownerId : ownerOrdersMap.keySet()) {
			...
			if(ownerId.intValue() == 1){
				bodyText = MarketESBConstant.BODY_TEXT;
	...
	private void emailOwner(String ownerEmailIds, StringBuilder sb, String bodyText, String ccEmail) {
		...
		body.append("There were problems on cancelling orders at NPS. See the attachement for order information. For working instructions click on the link  ");
