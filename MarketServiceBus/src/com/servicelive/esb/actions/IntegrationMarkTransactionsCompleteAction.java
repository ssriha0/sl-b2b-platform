...
public class IntegrationMarkTransactionsCompleteAction extends AbstractIntegrationSpringAction {

	private static final Logger logger = Logger.getLogger(IntegrationMarkTransactionsCompleteAction.class);
	private static final String INTEGRATION_ID_PROPERTY_NAME = "integrationId";
	private Long integrationId;
	private IntegrationName integrationName;
	
	public IntegrationMarkTransactionsCompleteAction() { super(); }
	public IntegrationMarkTransactionsCompleteAction(ConfigTree config) throws ConfigurationException {
		super(config);
	...
	public Message markTransactionsComplete(Message message) throws ActionProcessingException {
		final String methodName = "markTransactionsComplete";
		logger.info(String.format("Entered %s", methodName));
		logger.info("***Entering markTransactionsComplete in IntegrationMarkTransactionsCompleteAction***time of action***"+new Date()+"Thread: "+Thread.currentThread().getId());
		...
		if (inputFileName != null) {
			try {
				this.getIntegrationServiceCoordinator().markTransactionsCompleteByInputFile(inputFileName);
			} catch (Exception e) {
				throw new ActionProcessingException(String.format("Error while marking transactions complete."), e);
			}
		}

		logger.info(String.format("Exiting %s", methodName));
		long end = System.currentTimeMillis();
		logger.info("***Exiting markTransactionsComplete in IntegrationMarkTransactionsCompleteAction***time of action***"+new Date()+"Thread: "+Thread.currentThread().getId());
		logger.info("***Exiting markTransactionsComplete in IntegrationMarkTransactionsCompleteAction***Time taken***"+(end-start));
		return message;
	}
