...
import com.servicelive.esb.mapforce.NPSCancellationMapToRI_Input;
import com.servicelive.esb.service.ExceptionHandler;


public class NPSCancellationIntegrationFileGeneratorAction extends
		AbstractMapforceIntegrationAction {
	
	private static final Logger logger = Logger.getLogger(NPSCancellationIntegrationFileGeneratorAction.class);
	private static final String CLASS_NAME = NPSCancellationIntegrationFileGeneratorAction.class.getName();
	...
	private String outputFileNamePrefix = "INSOO";

	public NPSCancellationIntegrationFileGeneratorAction() {
		super();
	}

	public NPSCancellationIntegrationFileGeneratorAction(ConfigTree configTree) throws ConfigurationException {
		super(configTree);
	...
	public Message processMessage(Message message) throws ActionProcessingException {
		...
		Batch batchToBeProcessed = this.getIntegrationServiceCoordinator().getIntegrationBO().getCurrentAndPrepareNewOutgoingBatchFor(IntegrationName.RI_CANCEL_OUTBOUND);
		if (batchToBeProcessed == null) {
			logger.debug(String.format("Unable to find an open batch with unprocessed transactions for the '%s' integration. There is no work to do.", IntegrationName.RI_CANCEL_OUTBOUND));
		...
		try {
			...
			NPSCancellationMapToRI_Input cancellationMap = new NPSCancellationMapToRI_Input();
			com.altova.io.Output outputFile = new com.altova.io.FileOutput(outputFileName);
			cancellationMap.run(connection, batchToBeProcessed.getBatchId(), outputFile);
		} catch (Exception e) {
			throw new ActionProcessingException(
					String.format("Error occurred while trying to generate output file '%s' for batch: %s",
									outputFileName, batchToBeProcessed), e);
		}
		...
		try {
			...
			throw new ActionProcessingException(
					String.format("Unable to update processing status for batch: %s", batchToBeProcessed), e);
		}
	...
	public void exceptionHandler(Message message, Throwable throwable) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug(
				String.format(
					"Error occurred while trying to generate NPS Cancellation file '%s' from batch with id %d. The ESB message being processed was: %s",
	...
	protected Long getIntegrationId(String fileName) {
		return IntegrationName.RI_CANCEL_OUTBOUND.getId();
	}

	@Override
	protected String getIntegrationName(String fileName) {
		return IntegrationName.RI_CANCEL_OUTBOUND.toString();
	}
